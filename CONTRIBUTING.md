...
BizCore AI プロジェクトへの貢献を歓迎します！このガイドでは、プロジェクトへの貢献方法を説明します。
...
BizCore AI プロジェクトは、すべての貢献者に対して敬意と包括性を持って接することを期待しています。不適切な行動を発見した場合は、<team@bizcore.dev> までご報告ください。
...
# カバレッジの最小値を設定
pytest --cov=contracts --cov=infrastructure --cov=shared --cov=kernel --cov=harness --cov=control_plane --cov=domain --cov=apps --cov-fail-under=80
```
...
- [ ] 型エラーなし (`./check.sh type-check`)
- [ ] カバレッジ ≥ 80% (`pytest --cov` または `./check.sh test-cov`)
- [ ] フロントエンド(`control_plane/frontend`)ビルド成功 (`cd control_plane/frontend && npm run build`)
...
```bash
# 型チェックの詳細を確認
mypy contracts infrastructure shared kernel harness control_plane domain apps tests --show-error-codes
...
BizCore AI への貢献ありがとうございます！ 🎉

あなたの貢献が、BizCore AI をより良い基盤にします。
