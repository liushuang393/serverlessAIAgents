...
class TurnBasedCompressor:
    ...
    def add_message(
        ...
        self._logger.debug("メッセージ追加: turn=%d, role=%s, len=%d", self._turn_count, role.value, len(content))
    ...
    async def _generate_summary(self, messages: list[Message]) -> str:
        ...
        if self._summarizer:
            # LLM要約
            msg_dicts = [{"role": m.role.value, "content": m.content} for m in messages if m.role != MessageRole.SYSTEM]
            try:
