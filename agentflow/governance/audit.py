...
class AuditEvent(BaseModel):
    ...
    tool_call_id: str | None = Field(None, description="ツール呼び出しID")
    decision: str = Field(..., description="ガバナンス判定")
    reason: str = Field(..., description="判定理由")
    auth_decision: AuthDecision | None = Field(None, description="認可判定")
    auth_mode: AuthMode | None = Field(None, description="認可モード")
    auth_reason: str | None = Field(None, description="認可理由")
