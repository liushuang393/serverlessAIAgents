...
class MemoryImportanceTracker:
    ...
    def get_low_importance_memories(self, threshold: float = 0.1) -> list[str]:
        """获取低重要性记忆列表."""
        return [
            mid for mid, imp in self._importance.items() if self.get_importance(mid) < threshold
        ]
...
抽象知识:"""
    ...
    def get_stats(self) -> MemoryStats:
        ...
        if self._tracker._importance:
            self._stats.avg_importance = sum(self._tracker._importance.values()) / len(
                self._tracker._importance
            )
