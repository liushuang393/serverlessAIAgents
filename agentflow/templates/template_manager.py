        ...
        for param in metadata.parameters:
            ...
            if value is not None:
                if param.type == "string" and not isinstance(value, str):
                    raise ValueError(
                        f"Parameter {param.name} must be string, got {type(value)}"
                    )
                elif param.type == "int" and not isinstance(value, int):
                    raise ValueError(
                        f"Parameter {param.name} must be int, got {type(value)}"
                    )
                elif param.type == "bool" and not isinstance(value, bool):
                    raise ValueError(
                        f"Parameter {param.name} must be bool, got {type(value)}"
                    )
                elif param.type == "list" and not isinstance(value, list):
                    raise ValueError(
                        f"Parameter {param.name} must be list, got {type(value)}"
                    )
