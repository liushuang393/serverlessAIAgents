...
class Agent(ABC):
    ...
    async def process_with_metrics(self, input_text: str, context: Dict[str, Any] = None) -> str:
        """
        メトリクス収集付きで処理を実行する
...
class ResultAggregator(ABC):
    ...
    @abstractmethod
    async def aggregate(self, results: List[str], context: Dict[str, Any] = None) -> str:
        """
        複数の結果を統合する
    ...
    async def aggregate(self, results: List[str], context: Dict[str, Any] = None) -> str:
        ...
        for result in results:
            if result.strip():
                ...
                result_counts[normalized_result] = result_counts.get(normalized_result, 0) + 1
...
class ChainExecutor:
    ...
    async def execute_sequential(self, initial_input: str, context: Dict[str, Any] = None) -> ChainResult:
        """
        Agentを順次実行する
    ...
    async def execute_parallel(self, input_text: str, context: Dict[str, Any] = None) -> ChainResult:
        """
        Agentを並列実行する
        ...
        try:
            # 全Agentを並列実行
            tasks = [agent.process_with_metrics(input_text, context) for agent in self.agents]
