...
from ..core.models import Message, MessageRole, EvaluationResult
...
class EvaluatorOptimizer(Agent):
    ...
    async def _iterative_optimization(
        ...
        for iteration in range(self.max_iterations):
            ...
            # 品質チェック
            if attempt.evaluation.passed and attempt.evaluation.score >= self.min_score_threshold:
    ...
    async def _hybrid_optimization(
        ...
        # 品質基準を満たしていれば終了
        if (best_candidate.evaluation.passed and 
            best_candidate.evaluation.score >= self.min_score_threshold):
            logger.info(f"初期候補が品質基準を満たしました (スコア: {best_candidate.evaluation.score:.3f})")
            return best_candidate.content
        ...
        for iteration in range(self.max_iterations - 1):
            ...
            improved_attempt = await self._generate_content(
                current_prompt,
                context,
                initial_candidates + iteration + 1
            )
    ...
    async def _generate_content(
        ...
        messages = [
            Message(
                ...
                content="あなたは高品質な回答を生成する専門アシスタントです。"
            ),
    ...
    async def _evaluate_content(
        ...
        logger.debug(f"評価完了 (試行 {attempt.attempt_number}): "
    ...
    async def _generate_improvement_prompt(
        ...
        suggestions = "\n".join(f"- {suggestion}" for suggestion in attempt.evaluation.suggestions)
