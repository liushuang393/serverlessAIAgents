...
from ..core.models import Message, MessageRole, ChainResult
...
class WorkerAgent(Agent):
    ...
    async def execute_task(self, task: SubTask) -> Any:
        ...
        try:
            ...
            logger.info(f"ワーカー '{self.name}' がタスク '{task.id}' を完了しました "
...
class LLMWorker(WorkerAgent):
    ...
    async def _execute_task_impl(self, task: SubTask) -> str:
        ...
        messages = [
            ...
            Message(role=MessageRole.USER, content=f"タスク: {task.description}\n入力: {task.input_data}")
        ]
...
class DataProcessingWorker(WorkerAgent):
    ...
    def __init__(self, name: str, processor_func: Callable, config: Dict[str, Any] = None):
        """
        データ処理ワーカーを初期化する
...
class OrchestratorWorker(Agent):
    ...
    async def _default_result_aggregator(
        ...
        """
        successful_results = [
            task.result for task in results 
