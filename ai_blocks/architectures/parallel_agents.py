...
from .base import Agent, ResultAggregator, SimpleAggregator, ChainExecutor
...
class ParallelAgents:
    ...
    def __init__(
        ...
        logger.info(f"Parallel Agents '{self.name}' を初期化しました（{len(self.agents)}個のAgent）")
    ...
    async def process(self, input_text: str, context: Dict[str, Any] = None) -> str:
        ...
        try:
            logger.info(f"Parallel Agents '{self.name}' で{len(self.agents)}個のAgentを並列実行します")
            
            # 並列実行
            result = await self.executor.execute_parallel(input_text, context)
            
            # 成功したAgentの数をチェック
            success_count = sum(1 for r in result.intermediate_results if not r.get("error"))
            
            if success_count < self.min_success_count:
                logger.warning(f"成功したAgentが最小要求数({self.min_success_count})を下回りました: {success_count}")
                return "十分な数のAgentが成功しませんでした。"
            
            logger.info(f"Parallel Agents '{self.name}' が完了しました（成功: {success_count}/{len(self.agents)}）")
