...
class PromptChain:
    ...
    async def execute(self, initial_input: str, context: Dict[str, Any] = None) -> ChainResult:
        """
        チェーンを実行する
    ...
    async def _execute_with_retry(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if result.success:
                    logger.info(f"Prompt Chain '{self.name}' が成功しました（試行回数: {attempt + 1}）")
                    return result
                else:
                    last_result = result
                    if attempt < self.max_retries:
                        logger.warning(f"Prompt Chain '{self.name}' が失敗しました。再試行します（{attempt + 1}/{self.max_retries}）")
...
class TranslatorAgent(SpecializedAgent):
    ...
    def __init__(self, target_language: str = "English", llm_provider: Any = None, config: Dict[str, Any] = None):
...
def create_translation_chain(target_language: str = "English", llm_provider: Any = None) -> PromptChain:
    """翻訳チェーンを作成する"""
    agents = [
        AnalyzerAgent(llm_provider),
