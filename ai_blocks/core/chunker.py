...
class SimpleChunker(ChunkerInterface):
    ...
    async def _chunk_by_word(
        ...
        while start_word_idx < len(words):
            ...
            start_word_idx = max(start_word_idx + len(chunk_words) - overlap_words, start_word_idx + 1)
    ...
    async def _chunk_by_sentence(
        ...
        while start_sentence_idx < len(full_sentences):
            ...
            # 次の開始位置を計算（オーバーラップを考慮）
            overlap_sentences = min(overlap // 100, len(chunk_sentences))  # 概算でオーバーラップ文数を計算
            start_sentence_idx = max(start_sentence_idx + len(chunk_sentences) - overlap_sentences, start_sentence_idx + 1)
    ...
    async def _chunk_by_paragraph(
        ...
        while start_para_idx < len(paragraphs):
            ...
            start_para_idx = max(start_para_idx + len(chunk_paragraphs) - overlap_paras, start_para_idx + 1)
...
class SmartChunker(ChunkerInterface):
    ...
    async def chunk(
        ...
        if has_paragraphs and len(text) > chunk_size * 2:
            # 段落がある長いテキストの場合
            return await self.paragraph_chunker.chunk(text, chunk_size, overlap, metadata)
        elif has_sentences:
            # 文がある場合
            return await self.sentence_chunker.chunk(text, chunk_size, overlap, metadata)
        else:
            # その他の場合は文字数ベース
            return await self.character_chunker.chunk(text, chunk_size, overlap, metadata)
    ...
    async def merge_chunks(self, chunks: List[TextChunk]) -> str:
        ...
        """
        # 最初のチャンクの戦略を使用
        if chunks and chunks[0].metadata.get("strategy") == ChunkStrategy.PARAGRAPH.value:
            return await self.paragraph_chunker.merge_chunks(chunks)
        elif chunks and chunks[0].metadata.get("strategy") == ChunkStrategy.SENTENCE.value:
