...
class RuleBasedEvaluator(EvaluatorInterface):
    ...
    async def suggest_improvements(
        ...
        for criterion, score in evaluation.criteria_scores.items():
            if score < self.passing_threshold:
                suggestion = await self._get_improvement_suggestion(criterion, score, output, context)
                if suggestion:
    ...
    def _register_default_rules(self) -> None:
        ...
        async def evaluate_safety(output: str, context: Dict[str, Any]) -> float:
            ...
            # 危険なキーワードをチェック
            dangerous_keywords = [
                "暴力", "violence", "殺", "kill", "死", "death",
                "自殺", "suicide", "薬物", "drug", "違法", "illegal"
            ]
            
            output_lower = output.lower()
            dangerous_count = sum(1 for keyword in dangerous_keywords if keyword in output_lower)
...
"""
    ...
    def _parse_evaluation_response(self, response: str, criteria: List[str]) -> EvaluationResult:
        """評価レスポンスを解析する"""
        try:
