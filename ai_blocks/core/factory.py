...
import asyncio
import inspect
from typing import Dict, Any, Optional, Type, TypeVar, Generic, List, Callable, Union
from abc import ABC, abstractmethod
from dataclasses import dataclass, field
from contextlib import asynccontextmanager

from .registry import get_registry, ComponentRegistry, DeploymentStrategy
...
class ComponentFactory(Generic[T]):
    ...
    async def create(
        ...
        **kwargs
        ...
        # 設定をマージ
        merged_config = await self._merge_config(component_type, component_name, config or {})
    ...
    def _get_dependency_specs(self, component_type: str, component_name: str) -> List[DependencySpec]:
    ...
    async def _post_create(self, instance: Any, component_type: str, component_name: str) -> None:
        """インスタンス作成後の処理"""
        # 初期化処理
        ...
        # ヘルスチェック開始
        if hasattr(instance, 'start_health_check') and callable(instance.start_health_check):
            if asyncio.iscoroutinefunction(instance.start_health_check):
                await instance.start_health_check()
            else:
                instance.start_health_check()
    
    async def destroy(self, component_type: str, component_name: str, version: Optional[str] = None) -> None:
        """コンポーネントを破棄する"""
        key = f"{component_type}:{component_name}:{version or 'current'}"
...
class ArchitectureFactory(ComponentFactory):
    ...
    async def create_agent_router(
        ...
        return await self.create(
            ...
            config={
                'router_type': router_type or 'rule_based',
                **(config or {})
            }
        )
