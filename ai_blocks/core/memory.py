...
from .registry import register_component, DeploymentStrategy, HealthCheckResult
...
class InMemoryVectorStore:
    ...
    def add(self, id: str, vector: np.ndarray, content: str, metadata: Dict[str, Any] = None) -> None:
        """
        ベクトルを追加する
    ...
    def search(self, query_vector: np.ndarray, limit: int = 10, threshold: float = 0.0) -> List[Tuple[str, float]]:
        """
        類似ベクトルを検索する
    ...
    def get(self, id: str) -> Optional[Tuple[np.ndarray, str, Dict[str, Any], datetime]]:
        """
        IDでベクトルを取得する
...
class VectorMemory(MemoryInterface):
    ...
    def __init__(
        ...
        self.similarity_threshold = similarity_threshold or settings.memory_similarity_threshold
