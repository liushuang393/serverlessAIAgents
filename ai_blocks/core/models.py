...
class RouteResult(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(default_factory=dict, description="ターゲットに渡すパラメータ")
...
class EvaluationResult(BaseModel):
    ...
    criteria_scores: Dict[str, float] = Field(default_factory=dict, description="各基準のスコア")
...
class AgentConfig(BaseModel):
    ...
    parameters: Dict[str, Any] = Field(default_factory=dict, description="エージェント固有のパラメータ")


class ChainResult(BaseModel):
    """チェーン実行結果のデータモデル"""
    final_output: str = Field(..., description="最終出力")
    intermediate_results: List[Any] = Field(default_factory=list, description="中間結果のリスト")
