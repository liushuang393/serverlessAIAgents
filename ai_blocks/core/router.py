...
from .models import RouteResult, RouteDefinition
...
class RuleBasedRouter(RouterInterface):
    ...
    def register_route(self, route: RouteDefinition) -> None:
        ...
        if route.conditions.get("type") == RouteType.REGEX:
            try:
                self._compiled_patterns[route.pattern] = re.compile(route.pattern, re.IGNORECASE)
    ...
    async def _check_function_match(
        ...
        try:
            ...
            if isinstance(result, bool):
                ...
                if result > 0:
                    return (min(1.0, result), {}, f"カスタム関数マッチ: {match_function.__name__}")
            elif isinstance(result, tuple) and len(result) >= 2:
                confidence, parameters = result[:2]
                reasoning = result[2] if len(result) > 2 else f"カスタム関数マッチ: {match_function.__name__}"
                return (confidence, parameters, reasoning)
    ...
    def _add_default_routes(self) -> None:
        ...
        question_route = RouteDefinition(
            ...
            conditions={
                "type": RouteType.KEYWORD,
                "keywords": ["?", "？", "what", "how", "why", "when", "where", "何", "どう", "なぜ", "いつ", "どこ"]
            }
        )
