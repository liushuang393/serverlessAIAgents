...
class ObservabilityManager:
    ...
    def record_metric(self, name: str, value: float, metric_type: MetricType, labels: Dict[str, str] = None) -> None:
        """メトリクスを記録する"""
    ...
    def get_observability_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "types": {
                    metric_type.value: len([m for m in metrics if m.type == metric_type])
...
async def record_counter(name: str, value: float = 1.0, labels: Dict[str, str] = None) -> None:
    """カウンターメトリクスを記録する便利関数"""
    manager = await get_observability_manager()
...
async def record_histogram(name: str, value: float, labels: Dict[str, str] = None) -> None:
