...
from infrastructure.sandbox.tool_provider import tool
from kernel.agent_decorator import agent
    ...
    def __init__(self) -> None:
        ...
        self._llm = LLMClient(
            LLMConfig(
                ...
                api_key=str(llm_config.get("api_key")) if llm_config.get("api_key") is not None else None,
            ...
            "type_mappings": [
                {
                    ...
                    "type": self._target_adapter.get_type_mapping(v.get("type", ""), v.get("pic_clause", "")),
                }
...
"""
        ...
        response = await self._llm.generate(role="coding", prompt=prompt)
        java_code = response.content.strip() if isinstance(response.content, str) else ""
...
"""
        ...
        response = await self._llm.generate(role="coding", prompt=prompt)
        fixed_code = response.content.strip() if isinstance(response.content, str) else ""
