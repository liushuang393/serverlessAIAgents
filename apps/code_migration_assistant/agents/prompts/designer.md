...
```json
{
    ...
    "tables": [
      {
        ...
        "columns": [
          ...
          {"name": "customer_id", "type": "VARCHAR(10)", "cobol": "WS-CUSTOMER-ID"},
          {"name": "version", "type": "INTEGER", "note": "楽観ロック用、追加"},
  ...
  "error_handling": {
    "response_format": {"error": {"code": "string", "message": "string", "details": "array"}},
    "http_status_map": {
  ...
  "unknowns": [
    {"field": "COPY_CUSTDEF_content", "reason": "外部コピー句の内容が設計に影響する可能性"}
  ]
}
```
