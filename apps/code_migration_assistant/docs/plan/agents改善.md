...
※目的は **レガシーシステム移行（COBOL→Java / Struts→Spring Boot / JDKアップグレード等）を、工程化・検証中心・設定差し替えで汎用化** すること。
