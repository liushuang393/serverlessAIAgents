...
from agentflow.governance.enterprise_audit import EnterpriseAuditLogger, EnterpriseAuditEvent, AuditEventType, AuditSeverity
...
class CodeMigrationEngine(BaseEngine, SafetyMixin):
    ...
    async def _run_pipeline(self, inputs: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            await self._log_completion(MIGRATION_QUALITY_GATE, task_id, {"decision": quality_artifact.decision.value}, flow_context)
            ...
            report = self._compliance_reporter_agent.process({
                ...
                "fix": locals().get("fix_artifact", {}).model_dump(mode="json") if locals().get("fix_artifact") else {}
            })
            ...
            final_target_code = (
                locals().get("fix_artifact", {}).target_code if locals().get("fix_artifact") and locals().get("fix_artifact").applied
