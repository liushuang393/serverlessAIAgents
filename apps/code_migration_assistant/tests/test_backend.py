...
実行方法:
    pytest apps/code_migration_assistant/tests/test_backend.py -v
"""
...
def test_client(tmp_path: Path) -> TestClient:
    ...
    with (
        patch(
            "apps.code_migration_assistant.backend.migration_router.asyncio.create_task",
            side_effect=_fake_create_task,
        ),
        patch(
            "apps.code_migration_assistant.backend.migration_router._get_output_root",
