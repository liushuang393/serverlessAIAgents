...
必ず以下のJSON形式で出力してください：
{
    ...
    async def _diagnose_with_llm(self, question: str, constraints: list[str]) -> ClarificationOutput:
        """LLMを使用した診断."""
        user_prompt = f"""【質問】
{question}
...
上記の質問を深く診断し、JSON形式で出力してください。"""
        ...
        try:
            ...
            hidden_assumptions = [HiddenAssumption(**h) for h in data.get("hidden_assumptions", [])][:3]
    ...
    def _detect_hidden_assumptions(self, question: str, constraints: list[str]) -> list[HiddenAssumption]:
        """暗黙の仮定を検出."""
        assumptions = []
