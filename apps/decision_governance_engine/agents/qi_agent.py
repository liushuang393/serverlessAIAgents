...
JSON形式で出力してください。"""
        ...
        if data is None:
            self._logger.warning(
                f"QiAgent: JSONパース失敗 → デフォルト出力にフォールバック。 Response preview: {response[:200]}"
            )
            return self._create_default_output(shu_result)
        ...
        if not implementations:
            self._logger.warning("QiAgent: 必須フィールド implementations が空 → ShuResultからデフォルト生成")
        ...
        domain_techs = [self._normalize_domain_technology(dt) for dt in data.get("domain_technologies", [])[:5]]
        regulatory = [
            self._normalize_regulatory_consideration(rc) for rc in data.get("regulatory_considerations", [])[:5]
        ]
        geographic = [
            self._normalize_geographic_consideration(gc) for gc in data.get("geographic_considerations", [])[:5]
        ]
