...
【器】技術負債警告: {", ".join(input_data.qi_result.technical_debt_warnings) or "なし"}"""
        ...
        for f in raw_findings:
            ...
            for si in si_data:
                if isinstance(si, dict):
                    score_improvements.append(
                        ScoreImprovement(
                            target_score=self._truncate_text(
                                si.get("target_score", ""), self.MAX_TARGET_SCORE_LEN
                            ),
            ...
            findings.append(
                ReviewFinding(
                    ...
                    failure_point=self._truncate_text(
                        f.get("failure_point", ""), self.MAX_FAILURE_POINT_LEN
                    ),
                    impact_scope=self._truncate_text(
                        f.get("impact_scope", ""), self.MAX_IMPACT_SCOPE_LEN
                    ),
    ...
    def _generate_checkpoint_items(
        ...
        # 撤退基準 定義済み
        has_exit = bool(
            input_data.shu_result.exit_criteria
            and getattr(input_data.shu_result.exit_criteria, "exit_trigger", "")
        )
        items.append(
            CheckpointItem(
