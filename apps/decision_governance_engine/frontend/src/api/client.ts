...
export class DecisionApiError extends Error {
  ...
  static fromResponse(response: Response, details?: unknown): DecisionApiError {
    ...
    const message =
      statusMessages[response.status] || `APIエラー: ${response.statusText}`;
...
export class DecisionApiClient {
  ...
  streamDecision(
    ...
    if (stakeholders) {
      if (stakeholders.product_owner)
        params.set("stakeholder_product_owner", stakeholders.product_owner);
      if (stakeholders.tech_lead)
        params.set("stakeholder_tech_lead", stakeholders.tech_lead);
      if (stakeholders.business_owner)
        params.set("stakeholder_business_owner", stakeholders.business_owner);
      if (stakeholders.legal_reviewer)
        params.set("stakeholder_legal_reviewer", stakeholders.legal_reviewer);
    }
  ...
  async exportHtml(
    ...
    try {
      ...
      const blob = await response.blob();
      const contentType =
        response.headers.get("content-type") || blob.type || "text/html";
      const filename =
        parseFilenameFromDisposition(
          response.headers.get("content-disposition"),
  ...
  async signReport(reportId: string): Promise<SignatureResponse> {
    const response = await fetch(
      `${this.baseUrl}/api/report/${reportId}/sign`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
  ...
  async getSignature(
    reportId: string,
  ): Promise<{ signed: boolean; signature?: SignatureResponse["signature"] }> {
    const response = await fetch(
      `${this.baseUrl}/api/report/${reportId}/signature`,
      {
        credentials: "include",
      },
    );
  ...
  async recheckFinding(
    request: FindingRecheckRequest,
  ): Promise<FindingRecheckResponse> {
    const response = await this.fetchWithRetry(
   ...
   * 重要指摘に対する任意メモを保存.
   */
  async logFindingNote(
    request: FindingNoteRequest,
  ): Promise<FindingNoteResponse> {
    const response = await this.fetchWithRetry(
  ...
  ): Promise<CheckpointApplyResponse> {
    const response = await this.fetchWithRetry(
  ...
  async getHistory(options?: {
    ...
    if (options?.decision_role)
      params.append("decision_role", options.decision_role);
