...
export const KnowledgePage: React.FC<KnowledgePageProps> = ({ agentType }) => {
  ...
  const agentInfo =
    agentType === "shu"
      ? {
          name: t("knowledge.agent_shu_name"),
          label: t("knowledge.agent_shu_label"),
          icon: "📋",
          color: "indigo",
        }
      : {
          name: t("knowledge.agent_qi_name"),
          label: t("knowledge.agent_qi_label"),
          icon: "🔧",
          color: "violet",
        };
  ...
  const fetchDocuments = useCallback(async () => {
    ...
    try {
      ...
      // エラー詳細をUIに表示（型安全）
      const errorMessage =
        err instanceof Error ? err.message : t("knowledge.unknown_error");
  ...
  const handleDelete = async (docId: string) => {
    ...
    try {
      const res = await fetch(`/api/knowledge/${agentType}/${docId}`, {
        method: "DELETE",
      });
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-semibold text-lg">
                {agentInfo.name}・{t("knowledge.page_title")}
              </h1>
              <p className="text-xs text-slate-500">
                {agentInfo.label} {t("knowledge.page_subtitle")}
              </p>
            </div>
          </div>
          <button
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-[#12121a] rounded-xl border border-white/5 p-6 mb-8">
          <h2 className="text-sm font-medium text-slate-300 mb-4">
            <span aria-hidden="true">📚</span> {t("knowledge.add_new")}
          </h2>
          <div className="space-y-4">
            <div>
              ...
              >
                {agentType === "shu" ? (
                  <>
                    <option value="industry_practices">
                      {t("knowledge.topic_industry")}
                    </option>
                    <option value="case_studies">
                      {t("knowledge.topic_case_studies")}
                    </option>
                    <option value="methodology">
                      {t("knowledge.topic_methodology")}
                    </option>
                  </>
                ) : (
                  <>
                    <option value="technical_docs">
                      {t("knowledge.topic_technical_docs")}
                    </option>
                    <option value="compliance">
                      {t("knowledge.topic_compliance")}
                    </option>
                    <option value="architecture">
                      {t("knowledge.topic_architecture")}
                    </option>
                  </>
                )}
              </select>
            ...
            <div>
              ...
              <div className="text-xs text-slate-600 mt-1">
                {t("knowledge.chars_count").replaceAll(
                  "{count}",
                  String(newContent.length),
                )}
              </div>
            </div>
            <button
        ...
        <div className="bg-[#12121a] rounded-xl border border-white/5 p-6">
          <h2 className="text-sm font-medium text-slate-300 mb-4">
            ...
            {t("knowledge.registered").replaceAll(
              "{count}",
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <span
                        className={`text-xs px-2 py-0.5 bg-${agentInfo.color}-500/20 text-${agentInfo.color}-300 rounded mb-2 inline-block`}
                      >
                        {doc.topic}
                      </span>
