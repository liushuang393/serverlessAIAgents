...
export const KnowledgeDashboard: React.FC<Props> = ({ showCollectionForm }) => {
  const { t } = useI18n();
  const { collections, createCollection, deleteCollection, loading } =
    useKnowledgeStore();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard
          label={t("kb.stat_collections")}
          value={String(collections.length)}
          color="indigo"
        />
        <StatCard
          label={t("kb.stat_documents")}
          value={String(totalDocs)}
          color="emerald"
        />
        <StatCard
          label={t("kb.stat_active")}
          value={String(collections.length)}
          color="amber"
        />
          ...
          <div className="mb-6 bg-[#0a0a0f] rounded-lg border border-white/10 p-4 space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              <select
                ...
                onChange={(e) =>
                  setForm({ ...form, chunk_strategy: e.target.value })
                }
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, chunk_size: Number(e.target.value) })
                }
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, chunk_overlap: Number(e.target.value) })
                }
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-0.5 truncate">
                    {col.description || col.collection_name}
                  </p>
                  <div className="flex gap-4 mt-1 text-[10px] text-slate-600">
                    <span>
                      {t("kb.stat_documents")}: {col.document_count}
                    </span>
