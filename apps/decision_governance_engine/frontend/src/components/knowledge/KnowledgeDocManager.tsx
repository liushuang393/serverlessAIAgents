      ...
      {selectedCollection ? (
        <>
          ...
          >
            ...
            <p className="text-xs text-slate-600 mb-3">PDF, DOCX, CSV, TXT, MD, JSON</p>
            <button
          ...
          <div className="bg-[#12121a] rounded-xl border border-white/5 p-6">
            <h2 className="text-sm font-medium text-slate-300 mb-4">{t('kb.documents')}</h2>
            {loading ? (
              <div className="text-center py-6 text-slate-500">{t('common.loading')}</div>
            ) : documents.length === 0 ? (
              <div className="text-center py-6 text-slate-500">{t('kb.no_documents')}</div>
            ) : (
                ...
                {documents.map((doc) => (
                  ...
                  >
                    <div className="flex items-center gap-3 flex-1 min-w-0">
                      <span className="text-lg">{STATUS_ICON[doc.status] || '📄'}</span>
                    ...
                    <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => previewChunks(selectedCollection, doc.document_id)}
                      ...
                      {doc.status !== 'indexed' && (
                        <button
                          onClick={() => indexDocument(selectedCollection, doc.document_id)}
                      ...
                      <button
                        onClick={() => deleteDocument(selectedCollection, doc.document_id)}
            ...
            <div className="bg-[#12121a] rounded-xl border border-white/5 p-6">
              ...
              <div className="space-y-2 max-h-80 overflow-y-auto custom-scrollbar">
                {chunkPreviews.map((chunk) => (
                  <div key={chunk.index} className="bg-[#0a0a0f] rounded-lg p-3 border border-white/5">
                    <span className="text-[10px] text-indigo-400 font-mono">#{chunk.index}</span>
