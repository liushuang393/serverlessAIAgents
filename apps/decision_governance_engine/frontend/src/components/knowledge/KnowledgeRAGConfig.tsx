...
export const KnowledgeRAGConfig: React.FC = () => {
  const { t } = useI18n();
  const { collections, selectedCollection, setSelectedCollection } = useKnowledgeStore();
  ...
  const handleTestQuery = async () => {
    ...
    setQuerying(true);
    try {
      const result = await knowledgeApi.testQuery(selectedCollection, testQuery, topK);
      setQueryResult(result);
    } catch (e) {
      setQueryResult({ error: e instanceof Error ? e.message : 'Unknown error' });
    } finally {
      setQuerying(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#12121a] rounded-xl border border-white/5 p-4">
        <label className="block text-xs text-slate-500 mb-2">{t('kb.select_collection')}</label>
        <select
      ...
      {selected ? (
        <>
          ...
          <div className="bg-[#12121a] rounded-xl border border-white/5 p-6">
            <h2 className="text-sm font-medium text-slate-300 mb-4">{t('kb.current_settings')}</h2>
            <div className="grid grid-cols-2 gap-3">
              <ConfigRow label={t('kb.chunk_strategy')} value={selected.chunk_strategy} />
              <ConfigRow label={t('kb.chunk_size')} value={String(selected.chunk_size)} />
              <ConfigRow label={t('kb.chunk_overlap')} value={String(selected.chunk_overlap)} />
              <ConfigRow label={t('kb.retrieval_method')} value={selected.retrieval_method} />
              <ConfigRow label={t('kb.reranker')} value={selected.reranker || t('kb.none')} />
              <ConfigRow label="Top-K" value={String(selected.top_k)} />
              <ConfigRow label={t('kb.min_similarity')} value={String(selected.min_similarity)} />
              <ConfigRow label={t('kb.embedding_model')} value={selected.embedding_model || 'default'} />
            </div>
          </div>

          {/* テストクエリ */}
          <div className="bg-[#12121a] rounded-xl border border-white/5 p-6">
            <h2 className="text-sm font-medium text-slate-300 mb-4">{t('kb.test_query')}</h2>
