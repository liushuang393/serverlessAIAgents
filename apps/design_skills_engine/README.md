...
| パターン | 用途 | 経路 |
|----------|------|------|
| **B-1（同期パイプライン）** | デザイン画像生成パイプライン | Router → PipelineEngine → Intent → Plan → Generate |

- Engine: kernel `PipelineEngine` 使用
- 3 Agent: IntentAnalyzer → PromptPlanner → WorkflowExecutor
- フロントエンド: なし（API 専用の共有サービス）

## 使用手順

### 1. 前提条件

#### 1.1 BizCore 開発環境のインストール
...
```python
from kernel.skills.builtin.design_skills.agents.intent_analyzer_agent import IntentAnalyzerAgent
from kernel.skills.builtin.design_skills.agents.prompt_planner_agent import PromptPlannerAgent
...
### 5. BizCore skill として使用

```python
from kernel.skills.builtin.design_skills.design_skills import run
