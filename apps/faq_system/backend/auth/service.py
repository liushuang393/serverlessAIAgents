...
class AuthService:
    ...
    async def _authenticate_local_db_with_mfa(
        ...
        async with get_db_session() as session:
            ...
            if account.locked_until and account.locked_until > now:
                ...
                return (
                    False,
                    f"アカウントが一定時間ロックされています。あと {max(1, remaining)} 分後に再試行してください。",
                    None,
                )
            ...
            if account.mfa_enabled:
                ...
                # Check TOTP
                if not account.mfa_secret or not TimeBasedMFA.verify_totp(
                    account.mfa_secret, totp_code
                ):
    ...
    def _authenticate_ldap_sync(
        ...
        search_filter = os.getenv("FAQ_LDAP_USER_FILTER", "(uid={username})").format(
            username=username
        )
        conn.search(
            search_base=base_dn,
