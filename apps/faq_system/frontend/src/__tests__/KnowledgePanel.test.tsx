...
describe('KnowledgePanel', () => {
  ...
  it('開いている時にタブ名が表示される', () => {
    ...
    expect(screen.getByText('knowledge_panel.tab_dashboard')).toBeInTheDocument();
    expect(screen.getByText('knowledge_panel.tab_collections')).toBeInTheDocument();
    expect(screen.getByText('knowledge_panel.tab_documents')).toBeInTheDocument();
    expect(screen.getByText('knowledge_panel.tab_ingest')).toBeInTheDocument();
    expect(screen.getByText('knowledge_panel.tab_retrieval')).toBeInTheDocument();
  });
  ...
  it('閉じるボタンクリックで onClose が呼ばれる', async () => {
    ...
    const closeButton = screen.getByRole('button', { name: 'knowledge_panel.close' });
