...
describe('PanelCollections', () => {
  ...
  it('モックデータからコレクションカードが表示される', () => {
    ...
    expect(within(cardList).getByTestId('collection-card-faq_main')).toBeInTheDocument();
    expect(within(cardList).getByTestId('collection-card-docs_sub')).toBeInTheDocument();
  ...
  it('カードクリックで編集フォームが展開される', async () => {
    ...
    // フォームはまだ表示されていない
    expect(screen.queryByTestId('panel-collection-form')).not.toBeInTheDocument();
    ...
    expect(screen.queryByTestId('input-collection-name')).not.toBeInTheDocument();
  });
  ...
  it('保存ボタンが create モードで createCollection を呼ぶ', async () => {
    ...
    // collection_name を入力
    await user.type(screen.getByTestId('input-collection-name'), 'new_collection');
