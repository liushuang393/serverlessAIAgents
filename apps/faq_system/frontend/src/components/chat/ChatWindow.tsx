...
export const ChatWindow = () => {
  ...
  const { messages, sendMessage, isStreaming, lastQueryType, runtimeStatus } =
    useChatStore();
  const { ragEnabled, sqlEnabled, startPolling, stopPolling } =
    useServiceStatus();
  ...
  const quickActions = [
    {
      label: t("chat.quick.analyze_reports"),
      sub: t("chat.quick.analyze_reports_sub"),
      icon: FileText,
    },
    {
      label: t("chat.quick.sql_query"),
      sub: t("chat.quick.sql_query_sub"),
      icon: Database,
    },
    {
      label: t("chat.quick.tech_support"),
      sub: t("chat.quick.tech_support_sub"),
      icon: Wrench,
    },
    {
      label: t("chat.quick.sales_strategy"),
      sub: t("chat.quick.sales_strategy_sub"),
      icon: TrendingUp,
    },
  ];
  ...
  const runtimeText = isStreaming
    ...
    : hasMessages
      ...
      : "システム準備完了";
  const knowledgeState = !ragEnabled
    ? "OFF"
    : ["faq", "hybrid"].includes(lastQueryType ?? "")
      ? "実行"
      : "待機";
  const dataState = !sqlEnabled
    ? "OFF"
    : ["sql", "hybrid"].includes(lastQueryType ?? "")
      ? "実行"
      : "待機";
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(ellipse at 50% 0%, hsl(220, 20%, 11%), var(--bg-main))",
      }}
    >
      {/* Top Bar / Header Info */}
      <div
