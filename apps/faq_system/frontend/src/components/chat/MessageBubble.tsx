...
const AssistantRichContent: React.FC<{ message: ChatMessage }> = ({ message }) => {
    ...
    return (
        <div className="mt-2 space-y-1">
            ...
            {/* チャート */}
            {hasChart && (
                <Suspense fallback={<div className="h-80 flex items-center justify-center text-[var(--text-muted)]">チャート読込中…</div>}>
...
const QueryTypeBadge: React.FC<{ queryType?: string }> = ({ queryType }) => {
    ...
    const labels: Record<string, string> = { sql: 'SQL', hybrid: 'Hybrid', chat: 'Chat' };
    return (
