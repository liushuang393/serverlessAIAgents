...
const truncateTitle = (title: string, maxLen: number): string => {
  ...
  const segmenterCtor = (Intl as unknown as { Segmenter?: SegmenterCtor })
    .Segmenter;
...
export const Sidebar = ({ isOpen, onToggle }: SidebarProps) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`flex flex-col h-full w-[var(--sidebar-width)] transition-opacity duration-200 ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none select-none"}`}
      >
