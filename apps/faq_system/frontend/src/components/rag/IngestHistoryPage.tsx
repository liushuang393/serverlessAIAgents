...
export function IngestHistoryPage() {
  const { t } = useI18n();
  const { ingestRuns, ingestLoading, fetchIngestRuns, triggerIngest } =
    useRAGStore();
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span
                  className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs ${s.color} ${s.bg}`}
                >
                  <StatusIcon
                    size={12}
