  ...
  try {
    ...
    if (eventType === "progress") {
      return {
        ...
        query_type:
          typeof raw.query_type === "string" ? raw.query_type : undefined,
        skill: typeof raw.skill === "string" ? raw.skill : undefined,
        step: typeof raw.step === "number" ? raw.step : undefined,
        total_steps:
          typeof raw.total_steps === "number" ? raw.total_steps : undefined,
      };
    }
    if (eventType === "result") {
...
function formatProgressText(event: SseProgressEvent): string {
  ...
  const stepText =
    typeof event.step === "number" && typeof event.total_steps === "number"
  ...
  switch (event.type) {
    ...
    case "progress":
      setRuntimeStatus({
        ...
        totalSteps:
          typeof event.total_steps === "number" ? event.total_steps : 0,
      });
...
export const useChatStore = create<ChatState>((set, get) => ({
  ...
  deleteSession: async (sessionId) => {
    try {
      ...
      set((state) => ({
        ...
        lastQueryType:
          state.currentSessionId === sessionId ? null : state.lastQueryType,
        runtimeStatus:
          state.currentSessionId === sessionId ? null : state.runtimeStatus,
      }));
    } catch {
      /* 削除失敗時は UI 上で無視する */
    }
  ...
  updateLastAssistantMeta: (data) =>
    set((state) => {
      const msgs = [...state.messages];
      const last = msgs.at(-1);
      const queryType =
        typeof data.query_type === "string" ? data.query_type : null;
