...
async def test_list_sessions_with_data(client: httpx.AsyncClient) -> None:
    ...
    assert sessions[0]["title"] == "Message 0 in session…"
    assert len(sessions[0]["title"]) <= 21  # 20文字以内
...
@pytest.mark.asyncio
async def test_session_title_is_trimmed_to_20_chars(client: httpx.AsyncClient) -> None:
    """セッションタイトルは 20 文字以内に制限される."""
    long_text = "これはとても長いタイトル生成テストメッセージです"
    async with get_db_session() as session:
        session.add(
            ChatMessage(
                id="msg-session-title-long",
                session_id="session-title-20",
                user_id="user-session-test",
                username="tester",
                role="user",
                content=long_text,
    ...
    response = await client.get("/api/chat/sessions")
    assert response.status_code == 200
    target = next((s for s in response.json()["sessions"] if s["session_id"] == "session-title-20"), None)
