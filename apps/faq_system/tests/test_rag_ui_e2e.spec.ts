...
test.describe("FAQ System - RAG 管理 UI E2E", () => {
  ...
  test("新規コレクション作成フォームが開く", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="collection-form-modal"]'),
    ).toBeVisible();
  });

  test("コレクション作成 - FAQ Precision パターン自動セット", async ({
    page,
  }) => {
    await page.click('[data-testid="nav-rag"]');
    await page.click('[data-testid="rag-tab-collections"]');
    ...
    // 自動セット確認
    await expect(page.locator('[data-testid="chunk-strategy"]')).toHaveValue(
      "sentence",
    );
  ...
  test("検索テスト UI が動作する", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="test-query-input"]'),
    ).toBeVisible();
    ...
    // 結果エリアが表示される（空でも可）
    await expect(
      page.locator('[data-testid="query-result-area"]'),
    ).toBeVisible();
  });
  ...
  test("インジェスト履歴タブが開く", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="ingest-history-list"]'),
    ).toBeVisible();
  });
});
