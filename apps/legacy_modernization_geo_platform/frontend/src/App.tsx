...
function toTimelineEvent(event: StreamEvent, taskId: string): TaskEvent {
  const message =
    readEventString(event, "message", "reason", "error_message") ||
    event.event_type;
...
function renderA2UIComponent(
  ...
  if (component.type === "card") {
    const title =
      typeof props.title === "string" ? props.title : t("common.surfaceCard");
    return (
      <article key={componentKey} className="card a2ui-card">
        <h4>{title}</h4>
        {children.map((child, index) =>
          renderA2UIComponent(child, `${componentKey}-${index}`, t),
        )}
      </article>
    );
  }
...
export default function App() {
  ...
  const [surfaces, setSurfaces] = useState<
    Record<SurfaceKey, A2UIComponentNode[]>
  >(createEmptySurfaces());
    ...
    source.onmessage = (message) => {
      ...
      setTaskState((current) => {
        ...
        return {
          ...
          status: nextStatus,
          current_stage:
            readEventString(parsed, "stage") || current.current_stage,
        };
      });
      ...
      if (
        ...
        setSurfaces((current) => {
          ...
          if (parsed.event_type === "a2ui.component") {
            const component = readEventRecord(
              parsed,
              "component",
            ) as A2UIComponentNode | null;
            if (!component) {
              return current;
            }
            return {
  ...
  async function handleStart() {
    ...
    try {
      ...
      setStreamPath(
        response.stream_url || `/api/geo/${response.task_id}/stream`,
      );
  ...
  async function handleApproval(approved: boolean, action?: string) {
    ...
    try {
      const comment =
        action === "rewrite"
          ? t("content.rewriteApprovalComment")
          : t("approval.uiApprovalComment");
        ...
        {activeTab === "console" && (
          <section className="panel" data-testid="campaign-console">
                ...
                <input
                  ...
                  onChange={(event) =>
                    setFormState((current) => ({
                      ...
                      targets: {
                        ...current.targets,
                        legacy_stacks: event.target.value
                          .split(",")
                          .map((item) => item.trim())
                          .filter(Boolean),
                      },
            ...
            <div className="timeline" data-testid="event-log">
              {eventLog.length === 0 && (
                <p className="muted">{t("console.emptyTimeline")}</p>
              )}
              {eventLog.map((event) => (
                <article
            ...
            {surfaces.workspace.length > 0 ? (
              ...
              <div className="grid-two">
                <div className="card" data-testid="demand-signals-card">
                  ...
                  <p>
                    {artifacts.signal?.urgency_hypothesis ||
                      t("workspace.pendingResult")}
                  </p>
                  <ul>
                    {(artifacts.signal?.signals || []).map((signal) => (
                      <li key={`${signal.type}-${signal.source}`}>
                        {signal.description}
                      </li>
                    ))}
                  </ul>
                ...
                <div className="card" data-testid="question-map-card">
                  ...
                  <ul>
                    {(artifacts.questionGraph?.personas || []).map(
                      (persona) => (
                        <li key={persona.role}>
                          <strong>{persona.role}</strong>:{" "}
                          {persona.high_intent_questions[0] ||
                            persona.questions[0]}
                        </li>
                      ),
                    )}
                ...
                <div className="card" data-testid="evidence-summary-card">
                  ...
                  <ul>
                    {(artifacts.evidenceMatrix?.entries || [])
                      .slice(0, 4)
                      .map((entry) => (
              ...
              <div className="grid-two">
                <div className="card">
                  ...
                  <article data-testid="draft-preview">
                    <h5>
                      {artifacts.draft?.pages[0]?.title ||
                        t("content.pendingDraft")}
                    </h5>
                ...
                <div className="card">
                  <h4>{t("content.qa")}</h4>
                  <p data-testid="qa-risk-level">
                    {artifacts.qa?.risk_level || "--"}
                  </p>
                  <ul>
              ...
              pendingApprovals.map((approval: ApprovalRecord) => (
                ...
                >
                  ...
                  <div className="action-row">
                    ...
                    <button
                      type="button"
                      onClick={() => void handleApproval(false, "rejected")}
                    >
                      {t("approval.reject")}
                    </button>
                    <button
                      type="button"
                      onClick={() => void handleApproval(true, "rewrite")}
                    >
                      {t("approval.rewrite")}
                    </button>
                  </div>
            ...
            {surfaces.report.length > 0 ? (
              ...
              <div className="grid-two">
                <div className="card">
                  ...
                  <pre data-testid="report-markdown">
                    {taskState?.report?.markdown || t("report.pendingReport")}
                  </pre>
                </div>
                <div className="card">
