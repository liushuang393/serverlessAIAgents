...
test("covers operator analysis, rewrite, approval, publish, and public CTA flow", async ({
  page,
}) => {
  await startCampaign(
    page,
  ...
  await expect(
    page.getByTestId("evidence-summary-card").locator("li").first(),
  ).toBeVisible();
  ...
  await expect(page.getByTestId("published-page-link")).toBeVisible({
    timeout: 10000,
  });
  ...
  const href = await page
    .getByTestId("published-page-link")
    .getAttribute("href");
  ...
  if (href) {
    ...
    await expect(page.getByTestId("public-faq")).toContainText(
...
test("covers operator rejection flow before publishing", async ({ page }) => {
  ...
  await expect(page.getByTestId("task-status")).toContainText("failed", {
    timeout: 15000,
  });
  await page.getByTestId("tab-report").click();
  await expect(page.getByTestId("report-markdown")).toContainText(
