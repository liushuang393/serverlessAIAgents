...
export default function EvidenceViewer() {
  ...
  const fetchEvidences = useCallback(async () => {
    ...
    try {
      const [evidenceResp, guardResp] = await Promise.allSettled([
        apiClient.get<{ evidences: EvidenceItem[]; total: number }>(
          "/evidence",
        ),
  ...
  const diagnosticsById = (guard?.evidence_diagnostics || []).reduce<
    Record<string, EvidenceDiagnostic>
  >((acc, item) => {
  ...
  return (
    <Box>
          ...
          <CardContent>
            ...
            >
              ...
              <Chip
                ...
                label={
                  guard.status === "ready"
                    ? t("evidence.guard_ready")
                    : t("evidence.guard_needs_more")
                }
              />
            </Box>
            ...
            {filtered.map((evidence) => (
              <Fragment key={evidence.id}>
                <TableRow
                  ...
                  onClick={() =>
                    setExpandedId(
                      expandedId === evidence.id ? null : evidence.id,
                    )
                  }
                  sx={{ cursor: "pointer" }}
                >
                    ...
                    <Collapse in={expandedId === evidence.id}>
                      <Card variant="outlined" sx={{ m: 1 }}>
                        <CardContent>
                          ...
                          >
                            {(
                              (evidence.extracted_data?.keywords as string[]) ||
                              []
                          ...
                          <Typography variant="subtitle2">
                            {t("evidence.content_preview")}
                          </Typography>
                          <Typography variant="body2" color="text.secondary">
                            {String(
                              evidence.extracted_data?.content || "",
                            ).slice(0, 300)}
                            ...
                          </Typography>
      ...
      >
        {t("evidence.showing_count")
          .replaceAll("{total}", String(evidences.length))
          .replaceAll("{filtered}", String(filtered.length))}
      </Typography>
    </Box>
  );
}
