  ...
  const fetchPredictions = useCallback(async () => {
    ...
    try {
      const [predResp, calResp] = await Promise.allSettled([
        apiClient.get<{ predictions: PredictionItem[]; total: number }>(
          "/predictions",
        ),
        apiClient.get<{
      ...
      {calibrationBins.length > 0 && (
        <Card sx={{ mb: 3 }}>
          <CardContent>
            ...
            <Box
              sx={{ display: "flex", alignItems: "end", gap: 1, height: 100 }}
            >
                ...
                <Box key={bin.bin_range} sx={{ flex: 1, textAlign: "center" }}>
                  <Box
                    sx={{
                      ...
                      minHeight: 4,
                      bgcolor:
                        Math.abs(bin.avg_confidence - bin.avg_outcome) < 0.15
                          ? "success.main"
                          : "warning.main",
                      borderRadius: 1,
                      mb: 0.5,
                ...
                <TableCell>
                  ...
                  >
                    <Chip
                      ...
                      size="small"
                      sx={{
                        bgcolor: statusColors[pred.status] || "#666",
                        color: "white",
                        cursor: "help",
                      }}
                    />
