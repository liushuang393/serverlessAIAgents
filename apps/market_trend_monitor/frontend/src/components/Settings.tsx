...
function toAliasInputMap(
  ...
  competitors.forEach((name) => {
    ...
    const normalized = aliases.filter(
      (alias) =>
        alias.trim() && alias.trim().toLowerCase() !== name.toLowerCase(),
    );
...
const Settings: React.FC = () => {
  ...
  const [localCollectionWindowDays, setLocalCollectionWindowDays] =
    useState<number>(collectionWindowDays);
  const [showSuccess, setShowSuccess] = useState(false);
  const [noticeMessage, setNoticeMessage] = useState("");
  const [noticeSeverity, setNoticeSeverity] = useState<
    "success" | "warning" | "error"
  >("success");
  const [localCompetitors, setLocalCompetitors] = useState<string[]>([]);
  const [competitorInput, setCompetitorInput] = useState("");
  const [localCompetitorAliases, setLocalCompetitorAliases] = useState<
    Record<string, string>
  >({});
  ...
  useEffect(() => {
    const fetchCompetitorConfig = async () => {
      try {
        const response = await apiClient.get<CompetitorConfigResponse>(
          "/competitors/config",
        );
  ...
  const persistCompetitorConfig = async (
    ...
    const competitorAliases = nextCompetitors.reduce<Record<string, string[]>>(
      (acc, name) => {
  ...
  const handleCollect = async () => {
    ...
    const articlesCount = response.articles_count;
    const trendsCount = response.trends_count;
    const hasSummaryCounts =
      typeof articlesCount === "number" && typeof trendsCount === "number";
    ...
    if (hasSummaryCounts) {
      setNoticeMessage(
        t("stg.collect_success")
          ...
          .replaceAll("{articles}", String(articlesCount))
          .replaceAll("{trends}", String(trendsCount)),
      );
    } else {
      setNoticeMessage(response.message || t("stg.collecting"));
    }
    setShowSuccess(true);
  };
        ...
        >
          ...
          <Box mt={2}>
            <FormControl sx={{ minWidth: 220 }}>
              <InputLabel id="collection-window-label">
                {t("stg.collection_period")}
              </InputLabel>
              <Select
                ...
                onChange={(e) =>
                  setLocalCollectionWindowDays(Number(e.target.value))
                }
              >
                ...
                <Box mt={1} display="flex" flexDirection="column" gap={2}>
                  {localCompetitors.map((name) => (
                    <TextField
                      ...
                      onChange={(e) =>
                        handleAliasInputChange(name, e.target.value)
                      }
                      sx={{
                        "& .MuiInputBase-root": {
                          backgroundColor: "#0f1117",
