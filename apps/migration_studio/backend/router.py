...
COBOL移行パイプラインのREST APIエンドポイントを定義する。
...
from apps.migration_studio.backend.execution_adapter import (
...
from apps.migration_studio.pipeline.project import COBOLFile, COBOLProject
...
async def _run_legacy_pipeline(
    *,
    ...
    """既存 MigrationEngine で実行する（互換経路）."""

    from apps.migration_studio.output.packager import OutputPackager
    ...
    for cobol_file in cobol_files:
        async for event in engine.run_file(cobol_file):
            ...
            if event.event_type == "complete":
                program_name = event.data.get("program_name", "")
                if program_name:
                    try:
                        packager = OutputPackager(output_root)
                        zip_path = packager.create_download_package(program_name)
...
async def upload_cobol(
    ...
    try:
        ...
        return UploadResponse(
            ...
            program_names=[f.program_name for f in cobol_files],
        )
