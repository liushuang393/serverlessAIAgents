...
class OutputPackager:
    ...
    def create_download_package(
        ...
        if not program_dir.exists():
            raise FileNotFoundError(f"プログラムディレクトリが存在しません: {program_dir}")
        ...
        if not version_dir.exists():
            raise FileNotFoundError(f"バージョンディレクトリが存在しません: {version_dir}")
    ...
    def create_java_only_package(
        ...
        if not transform_dir.exists():
            raise FileNotFoundError(f"変換ディレクトリが存在しません: {transform_dir}")
    ...
    def _get_latest_version(program_dir: Path) -> int:
        ...
        if not versions:
            raise FileNotFoundError(f"バージョンが存在しません: {program_dir}")
