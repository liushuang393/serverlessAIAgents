...
実行方法:
    pytest apps/migration_studio/tests/test_cli_and_events.py -v
"""
...
from apps.migration_studio.cli import (
    _print_progress_cli,
    cmd_list,
    cmd_migrate,
    cmd_retry,
...
from apps.migration_studio.pipeline.engine import SSEEvent
...
class TestCmdList:
    ...
    def test_report_status_markers(self, tmp_path: Path, capsys) -> None:
        ...
        cmd_list(args)
        out = capsys.readouterr().out
        assert "✓" in out   # v1 は report あり
        assert "·" in out   # v2 は report なし
...
class TestCmdRetry:
    ...
    def test_all_valid_stages_pass_validation(self, tmp_path: Path, capsys) -> None:
        ...
        for stage in valid_stages:
            ...
            with patch("apps.migration_studio.pipeline.project.COBOLProject") as mock_project, \
                 patch("apps.migration_studio.pipeline.engine.MigrationEngine"):
...
class TestCmdMigrate:
    ...
    def test_success_returns_0_and_prints_result(self, tmp_path: Path, capsys) -> None:
        ...
        from apps.migration_studio.pipeline.engine import PipelineResult
        ...
        with patch("apps.migration_studio.pipeline.project.COBOLProject") as mock_project, \
             patch("apps.migration_studio.pipeline.engine.run_migration_sync", return_value=mock_result):
            mock_project.return_value.setup.return_value = None
            mock_project.return_value.get_cobol_files.return_value = [mock_cobol_file]
            result = cmd_migrate(args)
    ...
    def test_model_from_env_variable(self, tmp_path: Path) -> None:
        ...
        with patch("apps.migration_studio.pipeline.project.COBOLProject") as mock_project, \
             patch("apps.migration_studio.pipeline.engine.run_migration_sync", side_effect=capture), \
...
class TestSSEGeneratorFormat:
    ...
    async def test_events_have_id_field(self) -> None:
        ...
        from apps.migration_studio.backend.router import _sse_generator
        from apps.migration_studio.backend.task_store import TaskStore
