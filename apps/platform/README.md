...
- Windows/WSL 環境では、使用ポート（例: `8000`, `3100`, 各 app の `ports.api` / `ports.frontend`）を OS ファイアウォールで許可する

確認手順:

```bash
# 待受確認（ホスト側）
ss -ltnp | grep -E ':(8000|3100|3010|3002|3004)'

# 別マシンから疎通確認
curl http://<host-ip>:8000/health
curl http://<host-ip>:3100
```
...
bash apps/platform/scripts/setup_lan_demo.sh -HostIP 192.168.166.41 -Mode firewall-only
```
