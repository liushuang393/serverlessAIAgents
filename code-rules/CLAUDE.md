...
> **⚠️ このファイルは「詳細参照版」です。**
> 常時ロードされる最重要ルールは **`CLAUDE.md`（ルート）** に記載されています。
> このファイルは `CLAUDE.md` のトリガー表に従い、該当タスクが発生したときに読んでください。
...
## 🚨 Git 操作安全規約（AI エージェント厳守）

### ❌ 絶対禁止事項

- **无断的ワークスペース変更**: `git checkout`, `git reset`, `git restore`, `git stash` などの「ファイルの状態を変更する操作」を、ユーザーの明確な一行ごとの許可なしに行うこと。
- **一括適用**: 複数のファイルを一度に checkout したり、リモートの変更を勝手に「準備」してワークスペースに反映させること。
- **強制上書き**: `--force` や `-f` を伴う操作を独断で実行すること。

### ✅ 推奨される行動

- **非破壊的分析のみ**: 調査には `git diff`, `git log`, `git show`, `git ls-tree` などの「読み取り専用」コマンドのみを使用すること。
- **差分の提示**: 変更を反映する前に、必ず `git diff` の結果をチャット上で提示し、ユーザーの確認を仰ぐこと。
- **バックアップの徹底**: ユーザーの許可を得て操作を行う場合も、必ず事前に备份用ブランチを作成すること。
...
| 種別 | 推奨エンコーディング |
|------|-------------------|
| `.py`, `.md`, `.yaml`, `.json`, `.toml` | UTF-8 (BOM なし) |
| `.ps1`（日本語含む） | UTF-8 with BOM |
| `.html`, `.css`, `.js`, `.ts` | UTF-8 (BOM なし) |
| `.sh` | UTF-8 (BOM なし) |

## Framework Core Doctrine（必須）

`agentflow` コア実装は、以下 3 原則を常時満たすこと。

1. **外部フレームワーク依存を最小化する**
   - 外部 SDK / API 依存は Adapter / Provider 境界で吸収し、Core へ直接侵入させない。
   - 置換可能性を壊す実装（特定ベンダー固着、直結 import 乱立）を禁止する。
2. **積木/プラグイン式（高凝集・低結合）を優先する**
   - 機能追加は登録 API / インターフェース経由で行い、既存分岐へのベタ書き追加を避ける。
   - 設計判断は「柔軟・単純・抽象化優先」を基準にする。
3. **共通機能はテンプレート化し、業界標準インターフェースに寄せる**
   - 同種ロジックは再利用コンポーネント化し、重複実装を禁止する。
   - 入出力契約は標準的な型/プロトコルへ寄せ、将来の切替コストを最小化する。
...
このガイドの API 契約上の正本は `contracts/` (型定義) + `kernel/__init__.py` (re-export) とし、各サブモジュールは実装詳細とみなす。
...
依存方向: `apps → control_plane → domain → harness → kernel → shared → infrastructure → contracts`

- 上位層は下位層にのみ依存する。逆方向の import は禁止（boundary test で CI 検出）。
- `apps/` の業務ロジックを共有層に昇格させる場合、型定義は `contracts/` に、実装は `shared/` に配置する。
- UI 層から直接内部実装（例: `kernel.flow.executor`）を参照しない。公開 API を経由する。
- Lazy import（関数内 import）は層間境界の迂回策として許容されるが、モジュールスコープの逆依存は禁止。
...
- イベントは ad-hoc dict ではなく `kernel/protocols/agui_events.py` のモデルを優先する。
...
```python
from kernel import agent, AgentClient
...
6. 公開対象は `kernel/__init__.py` 経由で参照可能（型定義は `contracts/`）。
