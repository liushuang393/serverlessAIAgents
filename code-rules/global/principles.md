...
7. [Framework Core Doctrine](#framework-core-doctrine)
...
## 🧩 Framework Core Doctrine

`agentflow` の Core（`agentflow/*`）は、次の 3 原則を設計・実装・レビューの必須判定軸とする。

### 1. 外部依存の最小化（Less External Framework Dependency）

- 外部フレームワーク/SDK への依存は Adapter/Provider 境界に限定し、Core 内の直接依存を避ける。
- ベンダー固着を招く API 直結実装は避け、置換可能な抽象インターフェースを優先する。

### 2. 積木/プラグイン式（高凝集・低結合）

- 機能拡張はレジストリ/プラグイン登録 API と契約インターフェース経由で行う。
- 既存分岐の肥大化より、独立モジュール化と責務分離を優先する。
- 設計判断は「柔軟」「単純」「抽象化優先」を満たすこと。

### 3. 共通機能の封装・テンプレート化と標準整合

- 繰り返し使う機能は再利用可能な共通コンポーネントとして封装する（重複実装を避ける）。
- 入出力契約は業界標準の型・インターフェース・プロトコルに寄せる。
- 将来のフレームワーク切替やプロバイダー変更時に、呼び出し側修正を最小化できる設計を採用する。

---

## 🔧 環境設定管理（Environment Configuration）

### 基本原則

**host / port / API URL 等の環境依存値はソースコードにハードコードしない。**
環境変数（`.env`）または設定ファイルで一元管理し、環境ごとに切り替え可能にする。

### 環境分離の必須ルール

| ルール                 | 説明                                                                                                    |
| ---------------------- | ------------------------------------------------------------------------------------------------------- |
| **ハードコード禁止**   | ホスト名・ポート番号・API エンドポイント等の環境依存値をソースコードに直書きしない                      |
| **環境変数で一元管理** | `.env` / `.env.local` / `.env.production` 等で管理し、コード側は `process.env.*` や `os.environ` で参照 |

- `app_config.json` を持つ App は、host / port の正本を `app_config.json` に置き、`<APP>_HOST/PORT` は一時 override に限定する
- 起動優先順位は `明示指定（CLI 引数 / compose 注入 env） > app_config.json` とし、その後ろにコード側の第3既定値を置かない
- FastAPI backend の起動責務は共通 launcher またはそれを包む `scripts/dev.py` / Docker command に集約し、`main.py` / `api.py` に持たせない
  | **環境ごとの分離** | ローカル開発・テスト・本番で異なる値が必要な場合、環境別ファイルまたは変数名で明確に分離する |
  | **本番 ≒ テスト** | 本番環境はテスト環境と可能な限り同一構成にする。差異があるとテストで検出できないバグが本番で発生する |
  | **テンプレートの同期** | `.env.example` を変更したら、必ず `.env` も同様に変更する |

### LLM プロバイダー選択ルール（必須）

- LLM プロバイダー環境変数（`OPENAI_*` / `OLLAMA_*` など）が複数定義される可能性がある場合、**必ず `LLM_PROVIDER` を明示指定**する。
- `LLM_PROVIDER` を未指定の `auto` 判定は、ローカル検証や移行期間を除き常用しない。
- App 単位で上書きが必要な場合、App 専用 `.env` を使用し、フレームワーク既定値に暗黙依存しない。

### Host / Port の単一定義元ルール（必須）
...
```typescript
// ❌ 悪い例: vite.config.ts にポートとプロキシ先をハードコード
export default defineConfig({
  server: {
    port: 3000, // ハードコード
    proxy: {
      "/api": {
        target: "http://localhost:8000", // ハードコード
      },
    },
  },
});
...
├── .env                  # ローカル開発用（Git 管理対象外）
├── .env.test             # テスト環境用（任意）
└── .env.production       # 本番環境用（任意、CI/CD で注入推奨）
```

### 設定ファイルでの環境分岐（Vite の例）

```typescript
// vite.config.ts — 環境変数で API 先を切り替え
...
_最終更新: 2026-01-19 | BizCore 7コア層 + Apps外層対応_
