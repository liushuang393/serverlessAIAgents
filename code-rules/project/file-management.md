...
3. **生成ファイル (Docs)** → 対象プロジェクトの `docs/` に配置
4. **アプリケーションデータ (DB/Logs/Temp)** → 対象アプリ内の `data/`, `db/`, `temp/` に配置
5. **ルートディレクトリの衛生管理** → プロジェクトルートへの一時ファイル、DBファイル、テスト結果ファイルの直接生成を禁止
...
| **一時分析・ログ** | `.tmp/` | 一時 | 調査用・デバッグ用 |
| **アプリDB/データ** | `apps/{app_name}/data/` | 永続 | SQLite DB, 永続データ |
...
- ❌ プロジェクトルートに直接 `.db` ファイルや `.txt` テスト結果、トークンファイルを生成
- ❌ 無関係なドキュメントを生成
- ❌ 既存ドキュメントを無視して重複生成
- ❌ 英語でドキュメントを生成（日本語必須）
