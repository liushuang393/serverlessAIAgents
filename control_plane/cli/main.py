...
from control_plane.cli.commands.create import create
from control_plane.cli.commands.flow import flow
from control_plane.cli.commands.init import init
from control_plane.cli.commands.marketplace import marketplace
from control_plane.cli.commands.sandbox import sandbox
from control_plane.cli.commands.skills import skills
from control_plane.cli.commands.template import template
from control_plane.cli.commands.workspace import workspace
...
class BizCoreCLI(click.Group):
    ...
    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        """
        # タイトル
        title = Text("BizCore CLI", style="bold cyan")
...
@click.group(cls=BizCoreCLI)
@click.version_option(version="0.2.0", prog_name="bizcore")
@click.option(
    "--verbose",
    "-v",
    is_flag=True,
...
def cli(ctx: click.Context, verbose: bool) -> None:
    """BizCore - Lightweight AI Agent Development Framework.
    ...
    使用例:
        ...
        \b
        # エージェントを実行
        $ bizcore run my-agent

        \b
        # エージェントをマーケットプレイスに公開
        $ bizcore publish my-agent
    """
    # コンテキストオブジェクトを初期化
...
def run(
    ...
    例:
        ...
        bizcore run my-agent --input '{"text": "Hello"}'

        # 方式2: @agent デコレータ（v0.2.0 NEW）
        bizcore run . --agent-name QAAgent --input '{"question": "Hello"}'

        # 方式3: ストリームモード
        bizcore run my-agent --input data.json --stream
    """
...
def list_agents(ctx: click.Context) -> None:
    ...
    if table.rows:
        ...
        console.print("[dim]Tip: Use 'bizcore create agent <name>' to create a new agent[/dim]")
...
def chat(ctx: click.Context, system_prompt: str, model: str) -> None:
    ...
    例:
        bizcore chat
        bizcore chat --system "あなたはコード専門家です"
        bizcore chat --model gpt-4o
    """
    from kernel.skills import ChatBotConfig, ChatBotSkill

    console.print(
        Panel(
            "[cyan]BizCore Chat[/cyan]\n[dim]終了するには 'exit' または 'quit' と入力してください[/dim]",
            border_style="cyan",
        )
    )
...
def studio(ctx: click.Context, host: str, port: int, reload: bool) -> None:
    """BizCore Studio サーバーを起動.

    例:
        bizcore studio
        bizcore studio --port 3000
        bizcore studio --reload
    """
    import uvicorn
    ...
    app = create_app()
    uvicorn.run(
        app if not reload else "control_plane.ui.studio.api:create_app",
        host=host,
        port=port,
        reload=reload,
        factory=reload,
...
def handle_error(error: Exception, verbose: bool = False) -> None:
    ...
    console.print()
    console.print("[dim]ヘルプ: bizcore --help[/dim]")
