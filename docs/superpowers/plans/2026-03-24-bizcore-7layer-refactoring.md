...
**優先度 MEDIUM（scope creep = shared に置くべきでないモジュール）:** 2. `shared/memory/` (7ファイル) — 将来的に kernel/memory/ へ移動候補 3. `shared/skills/` — 将来的に kernel/skills/builtin/ へ移動候補 4. `shared/rag/` — 将来的に整理

**優先度 LOW（infrastructure wrapper として許容可能）:** 5. `shared/gateway/` (Task 3.2 で DI 化済み) 6. `shared/config/`, `shared/trace/`
