...
from ai_blocks.core.factory import get_factory, builder
from ai_blocks.config.dynamic import get_config_manager, config_context
...
async def demo_hotswap_capabilities():
    """ホットスワップ機能のデモ"""
    await demo_section_header(
        "ホットスワップ機能デモ",
        "実行時でのコンポーネント交換とバージョン管理"
    )
    ...
    if success:
        ...
        print(f"   v2.0.0 検索結果: {len(results)}件（改良された類似度: {results[0].similarity_score if results else 'N/A'}）")
    else:
        print("   ❌ ホットスワップ失敗")
...
async def demo_observability_features():
    ...
    async def complex_operation():
        ...
        with observability.trace("tool_operations"):
            calc_result = await tool_manager.execute("demo_calculator", {"expression": "10 + 20"})
    ...
    # 様々なメトリクスを記録
    observability.record_metric("demo_requests_total", 1.0, "counter", {"endpoint": "/api/demo"})
    observability.record_metric("demo_active_users", 42.0, "gauge")
    observability.record_metric("demo_response_time", 0.123, "histogram", {"method": "GET"})
    ...
    with observability.trace("integrated_workflow_demo"):
        ...
        for query in queries:
            with observability.trace(f"query_processing_{query}"):
                ...
                observability.record_metric(
                    "query_results_count", len(results), "gauge",
                    {"query": query}
                )
        ...
        for i in range(20):
            task = asyncio.create_task(
                memory.search(f"テスト{i % 3}", limit=2)
            )
            tasks.append(task)
        ...
        # パフォーマンスメトリクスを記録
        observability.record_metric("batch_query_duration", end_time - start_time, "histogram")
