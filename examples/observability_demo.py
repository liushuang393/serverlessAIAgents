...
from ai_blocks.utils.observability import (
    get_observability_manager, trace, record_counter, record_gauge, record_histogram,
    MetricType, SpanStatus
)
...
async def simulate_llm_request(prompt: str, model: str = "gpt-3.5-turbo") -> str:
    ...
    with manager.trace("llm_processing", {"model": model, "prompt_length": len(prompt)}):
        await asyncio.sleep(processing_time)
        
        # 応答時間をヒストグラムに記録
        await record_histogram("llm_response_time_seconds", processing_time, {"model": model})
        ...
        if random.random() < 0.1:  # 10%の確率でエラー
            await record_counter("llm_errors_total", labels={"model": model, "error_type": "timeout"})
...
async def simulate_memory_operations() -> None:
    ...
    with manager.trace("memory_store_batch"):
        ...
        for i, doc in enumerate(documents):
            ...
            await record_counter("memory_operations_total", labels={"operation": "store"})
    ...
    with manager.trace("memory_search_batch"):
        ...
        for query in queries:
            ...
            await record_gauge("memory_search_results_count", len(results), {"query_type": "similarity"})
            await record_counter("memory_operations_total", labels={"operation": "search"})
    ...
    for tool_name, params in tools_to_test:
        with manager.trace(f"tool_execution_{tool_name}", {"tool": tool_name}):
            ...
            try:
                ...
                # 成功メトリクス
                await record_histogram("tool_execution_duration_seconds", execution_duration, {"tool": tool_name})
                await record_counter("tool_executions_total", labels={"tool": tool_name, "status": "success"})
                
                if result.success:
                    await record_gauge("tool_last_execution_success", 1, {"tool": tool_name})
                else:
                    await record_gauge("tool_last_execution_success", 0, {"tool": tool_name})
                    await record_counter("tool_errors_total", labels={"tool": tool_name, "error_type": "execution_failed"})
                ...
                # エラーメトリクス
                await record_histogram("tool_execution_duration_seconds", execution_duration, {"tool": tool_name})
                await record_counter("tool_executions_total", labels={"tool": tool_name, "status": "error"})
                await record_counter("tool_errors_total", labels={"tool": tool_name, "error_type": "exception"})
                await record_gauge("tool_last_execution_success", 0, {"tool": tool_name})
...
async def demo_metrics_collection():
    ...
    # カウンターメトリクス
    for i in range(10):
        await record_counter("demo_requests_total", labels={"endpoint": f"/api/v{i%3+1}"})
    ...
    # ヒストグラムメトリクス
    for _ in range(20):
        await record_histogram("demo_request_duration_seconds", random.uniform(0.1, 2.0))
    ...
    for metric in metrics[:10]:  # 最初の10件を表示
        labels_str = ", ".join(f"{k}={v}" for k, v in metric.labels.items()) if metric.labels else "なし"
        print(f"   {metric.name} ({metric.type}): {metric.value:.3f} [ラベル: {labels_str}]")
...
async def demo_performance_monitoring():
    ...
    if history:
        ...
        for i, metrics in enumerate(history, 1):
            ...
            print(f"   {i}. {timestamp} - CPU: {metrics.cpu_usage:.1f}%, メモリ: {metrics.memory_usage:.1f}%")
...
async def demo_observability_summary():
    ...
    print(f"   タイムスタンプ: {time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(summary['timestamp']))}")
