    ...
    def _register_routes(self) -> None:
        ...
        async def create_vector_index(request: CreateIndexRequest) -> Dict[str, str]:
            ...
            try:
                ...
                chunk_overlap = request.chunk_overlap or self.config["default_chunk_overlap"]
                ...
                # ドキュメントを解析
                documents = self._parse_documents(request.file_path, chunk_size, chunk_overlap)
        ...
        async def get_document_summary(request: SummaryRequest) -> Dict[str, Any]:
            ...
            try:
                if request.index_name not in self.indices:
                    raise HTTPException(
                        status_code=404,
                        detail=f"インデックス {request.index_name} が存在しません"
                    )
