...
class SimpleRAGTest:
    ...
    def _get_document_hash(self, file_path: str, chunk_size: int, chunk_overlap: int) -> str:
        ...
        try:
            ...
            return hashlib.md5(f"{file_path}_{chunk_size}_{chunk_overlap}".encode()).hexdigest()
    
    def _parse_documents(self, file_path: str, chunk_size: int, chunk_overlap: int) -> List[str]:
        """ドキュメントを解析（簡略版）"""
    ...
    def create_vector_index(self, file_path: str, index_name: str,
                           chunk_size: Optional[int] = None, chunk_overlap: Optional[int] = None,
                           force_recreate: bool = False) -> Dict[str, str]:
        """ベクトルインデックスを作成（簡略版）"""
        try:
            # デフォルト設定を使用
            ...
            if not documents:
                return {
                    "status": "error",
                    "message": f"ドキュメントの解析に失敗しました: {file_path}"
                }
            ...
            return {
                "status": "error",
                "message": f"インデックス作成失敗: {str(e)}"
            }
    
    def query_document(self, index_name: str, query: str, top_k: int = 5) -> Dict[str, Any]:
        """ドキュメントを検索（簡略版）"""
        try:
...
def main() -> None:
    ...
    # テストドキュメントのパス
    test_docs = [
        "test_documents/ai_report.txt",
        "test_documents/market_analysis.txt"
    ]
