...
def get_checkpointer(backend: str | None = None) -> Checkpointer:
    ...
    環境変数・引数に応じて適切な Checkpointer 実装を返す。

    Args:
        backend: "memory", "redis", "postgres" のいずれか。
                 None の場合は環境変数から自動検出。

    Returns:
        Checkpointer 実装インスタンス
    """
    import os

    if backend is None:
        backend = os.environ.get("CHECKPOINTER_BACKEND", "").lower()
        if not backend:
            # 環境変数から自動検出
    ...
    if backend == "redis":
        try:
            ...
            logger.warning("Redis Checkpointer の初期化に失敗。MemoryCheckpointer にフォールバック。")
    ...
    if backend == "postgres":
        try:
            ...
            logger.warning("Postgres Checkpointer の初期化に失敗。MemoryCheckpointer にフォールバック。")
            return MemoryCheckpointer()
