...
class AgentModelMapping(BaseModel):
    ...
    default_model: str = Field(default="platform_text_default", description="デフォルトモデル")
...
class RouterConfig(BaseModel):
    ...
    fallback_models: list[str] = Field(
        ...
        description="フォールバックモデル（ゲートウェイルーティングポリシーで管理）",
    )
...
class MultiLLMRouter:
    ...
    async def _rank_candidates(self, candidates: list[str]) -> str:
        ...
        Returns:
            最適なモデル名
        """
        from infrastructure.llm.models import MODELS, ModelTier
