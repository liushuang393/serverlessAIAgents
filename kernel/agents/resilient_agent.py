...
Agent 定義方式（2つのみ）:
    方式1: 手動コーディング — ResilientAgent を継承して process() を実装
...
class ResilientAgent[InputT: BaseModel, OutputT: BaseModel](ABC):
    """健壮な Agent 基底クラス.

    Agent 定義の唯一の標準基底クラス。全ての Agent はこのクラスを継承する。
    ...
    - ライフサイクル管理 (initialize / cleanup / async context manager)

    クラス属性（サブクラスで設定）:
        name: Agent 名（必須）
        timeout_seconds: タイムアウト秒数（デフォルト: 300）
        max_retries: 最大リトライ回数（デフォルト: 3）
        retry_delay: リトライ間隔秒（デフォルト: 2.0）
        retry_backoff: リトライ退避方式 "fixed" | "exponential"（デフォルト: "exponential"）
        max_tokens: LLM 最大トークン数（デフォルト: 4000）
    ...
    async def initialize(self) -> None:
        ...
        if self._tool_catalog is None:
            try:
                get_tool_catalog = importlib.import_module("kernel.tools.tool_catalog").get_tool_catalog
    ...
    async def run(self, input_data: dict[str, Any]) -> dict[str, Any]:
        ...
        # 全リトライ失敗
        exceptions_module = importlib.import_module("kernel.core.exceptions")
        agent_retry_exhausted_error = exceptions_module.AgentRetryExhaustedError
        agent_timeout_error = exceptions_module.AgentTimeoutError

        if isinstance(last_error, (asyncio.TimeoutError, TimeoutError)):
            raise agent_timeout_error(
                self.name,
                self.timeout_seconds,
                attempts_used,
                last_error,
            )
        raise agent_retry_exhausted_error(
            self.name,
            attempts_used,
            last_error,
        )
    ...
    async def _run_with_timeout(self, input_data: dict[str, Any], attempt: int) -> dict[str, Any]:
        ...
        async with asyncio.timeout(self.timeout_seconds):
            ...
            try:
                ...
                if isinstance(e, (ValidationError, agent_output_validation_error)):
    ...
    async def emit_progress(self, text: str) -> None:
        ...
        if ctx is not None and hasattr(ctx, "emit_status"):
            ...
            a2a_task_state = A2ATaskState
            await ctx.emit_status(a2a_task_state.WORKING, text)
    ...
    # ========================================
    # ライフサイクル管理（旧 AgentBlock から移動）
    # ========================================

    async def cleanup(self) -> None:
        """エージェントをクリーンアップ（オプション）.

        サブクラスでオーバーライドして、接続のクローズ等を実装できる。
        """
        self._initialized = False
    ...
    async def __aenter__(self) -> ResilientAgent[InputT, OutputT]:
...
# Agent は ResilientAgent のエイリアス（推奨名称）
