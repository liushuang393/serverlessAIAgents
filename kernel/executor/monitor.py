...
使用例:
    >>> from kernel.executor.monitor import MonitorAgent
    >>>
...
class MonitorAgent:
    ...
    def _emit_event(self, event: MonitorEvent) -> None:
        ...
        # アラートコールバック
        if self._alert_callback and event.severity in (AlertSeverity.ERROR, AlertSeverity.CRITICAL):
            try:
