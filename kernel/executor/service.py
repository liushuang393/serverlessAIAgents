"""実行Agent - 計画ステップの実行（Layer 3 Kernel）.
...
class ExecutorAgent:
    ...
    async def execute_step(
        ...
        self._logger.info("ステップ実行開始: %s (%s)", step.name, step.step_type.value)
        ...
        while retries <= self._config.max_retries:
            try:
                ...
                self._logger.info("ステップ完了: %s (%.0fms)", step.name, duration_ms)
                return result
        ...
        await self._safe_record(
            ...
            self._run_id(context),
            step.id,
            {"duration_ms": duration_ms, "retries": retries - 1, "error": last_error},
        )

        self._logger.error("ステップ最終失敗: %s - %s", step.name, last_error)
        return result
    ...
    async def _execute_tool_call(
        ...
        await self._safe_record(
            ...
            self._run_id(context),
            step.id,
            {"tool_uri": step.tool_uri, "params": params, "success": result.success},
        )
    ...
    async def _execute_llm_generation(
        ...
        await self._safe_record(
            ...
            self._run_id(context),
            step.id,
            {"prompt": prompt, "response_preview": str(response)[:500]},
        )
        return response
    ...
    async def _execute_human_input(
        ...
        self._logger.info("人間入力待ち: %s", step.name)
        await asyncio.sleep(0)  # async 準拠: 将来的にイベントベースの入力待ちに置換
