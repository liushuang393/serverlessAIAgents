...
Example:
    >>> from kernel.protocols.ucp import UCPClient
    >>> async with UCPClient(config) as client:
    ...     intent = await client.analyze_intent("ノートPCが欲しい")
    ...     offers = await client.get_offers(intent)
"""

from kernel.protocols.ucp.ucp_adapter import UCPAdapter
from kernel.protocols.ucp.ucp_client import UCPClient
from kernel.protocols.ucp.ucp_config import (
    UCPConfig,
    UCPEndpoint,
    UCPSecurityConfig,
)
from kernel.protocols.ucp.ucp_messages import (
    UCPError,
    UCPIntentRequest,
    UCPIntentResponse,
    UCPMessage,
