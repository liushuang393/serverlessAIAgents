...
class ResultVerifier:
    ...
    _logger: logging.Logger = field(default_factory=lambda: logging.getLogger("kernel.reviewer"))
    ...
    async def verify(
        ...
        self._logger.info("検証開始: %s (strategy=%s)", goal, strategy.value)
        ...
        self._logger.info("検証完了: 合格=%s, スコア=%.2f", verification.is_acceptable, verification.score)
...
{{
    ...
    "suggestions": ["改善提案1", "改善提案2"]
        ...
        try:
            ...
            self._logger.warning("意味的検証失敗: %s", e)
            return self._verify_threshold(result, expected)
        ...
        for constraint in constraints:
            ...
            if constraint_type == "min":
                ...
                if value is not None and value >= threshold:
                    ...
                    violations.append(f"{field_name} >= {threshold} (実際: {value})")
                ...
                if value is not None:
                    ...
                    violations.append(f"{field_name} は必須です")
                ...
                if value in allowed:
                    ...
                    violations.append(f"{field_name} は {allowed} のいずれかである必要があります")
...
{{
    ...
    "replan_reason": "再計画が必要な場合の理由"
        ...
        try:
            ...
            self._logger.warning("LLM判定失敗: %s", e)
            return self._verify_threshold(result, expected)
    ...
    async def _verify_composite(
        ...
        return VerificationResult(
            ...
            details={
                "strategy_results": [
                    {"strategy": r["strategy"], "score": r["result"].score, "weight": r["weight"]} for r in results
                ]
            },
        )
