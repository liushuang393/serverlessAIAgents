...
def _build_request(input_data: dict[str, Any]) -> WebRouterInput:
    ...
    return WebRouterInput(
        request_id=str(input_data.get("request_id", "web-crawl")),
        intent=WebIntent(
            intent=WebIntentType.CRAWL,
            urls=urls if isinstance(urls, list) else None,
            url=str(input_data.get("url", "")) if input_data.get("url") else None,
        ),
...
def crawl(input_data: dict[str, Any]) -> dict[str, Any]:
