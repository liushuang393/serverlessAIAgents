...
class SkillRuntime:
    ...
    async def run(
        ...
        *,
        ...
        """Skill のスクリプトを自動選択して実行.

        スクリプト選択の優先順位:
        1. skill名と同名のスクリプト (例: web-content-fetcher → fetch.py 不可なら次)
        2. main.py / main.sh
        3. scripts/ 内の唯一のスクリプト
        4. scripts/ 内の最初のスクリプト
    ...
    def _auto_select_script(self, skill: Skill) -> str | None:
        ...
        Returns:
            スクリプト名（拡張子なし）、見つからない場合 None
        """
        scripts = self.list_scripts(skill)
        if not scripts:
            return None

        # 1. skill名の末尾部分にマッチ (web-content-fetcher → "fetch")
    ...
    async def _execute_subprocess(
        ...
        動的インポート (exec_module) が sys.modules 汚染等で失敗した場合の
        フォールバック。クリーンなPythonプロセスで実行するため依存問題を回避。
