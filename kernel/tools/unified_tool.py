...
使用例:
    >>> from kernel.tools.unified_tool import UnifiedToolProvider
    >>>
...
class ToolProvider(ABC):
    ...
    @abstractmethod
    def get_tool(self, tool_name: str) -> UnifiedToolDefinition | None:
        """ツール定義を取得."""
        ...
...
class SkillToolProvider(ToolProvider):
    ...
    def __init__(self) -> None:
        """初期化."""
        self._skills: dict[str, UnifiedToolDefinition] = {}
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            self._logger.info("Skillsプロバイダー初期化完了: %d件", len(self._skills))
        except Exception:
            self._logger.warning("Skillsプロバイダー初期化失敗", exc_info=True)
...
class MCPToolProvider(ToolProvider):
    ...
    async def initialize(self, config: Any = None) -> None:
        ...
        try:
            ...
            self._logger.info("MCPプロバイダー初期化完了: %d件", len(self._tools))
        except Exception:
            self._logger.warning("MCPプロバイダー初期化失敗", exc_info=True)
...
class BuiltinToolProvider(ToolProvider):
    ...
    def register_tool(
        ...
        self._logger.debug("内蔵ツール登録: %s", definition.name)
...
class UnifiedToolProvider:
    ...
    async def call(
        ...
        self._logger.debug("ツール呼び出し: %s (type=%s)", tool_uri, tool_type)
    ...
    def register_custom_provider(
        ...
        self._logger.info("カスタムプロバイダー登録: %s", name)

    def list_all_tools(self) -> list[UnifiedToolDefinition]:
        """全ての利用可能なツールを一覧取得.

        Returns:
            ツール定義リスト
        """
        all_tools: list[UnifiedToolDefinition] = []
    ...
    def get_tool(self, tool_uri: str) -> UnifiedToolDefinition | None:
        """ツール定義を取得.
