"""Layer 5 Platform API の正規エントリーポイント."""
...
def _load_runtime_defaults() -> tuple[str, int]:
    ...
    config_path = Path(__file__).resolve().parents[2] / "apps" / "platform" / "app_config.json"
...
def _load_publish_contracts() -> tuple[type[Any], type[Any]]:
    ...
    request_type = _load_legacy_symbol("platform.publish.contracts", "PublishRequest")
    target_type = _load_legacy_symbol("platform.publish.contracts", "PublishTarget")
    return request_type, target_type


def _include_legacy_routers(app: FastAPI) -> None:
    """既存 routers を登録する。"""
    routers = importlib.import_module("platform.api.routers")
    router_names = (
        "gallery_router",
        "components_router",
        "publish_router",
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    app_discovery_cls = _load_legacy_symbol("platform.discovery.runtime", "AppDiscoveryService")
    app_lifecycle_cls = _load_legacy_symbol("platform.lifecycle.runtime", "AppLifecycleManager")
    app_scaffolder_cls = _load_legacy_symbol("platform.discovery.runtime", "AppScaffolderService")
    port_allocator_cls = _load_legacy_symbol("platform.lifecycle.runtime", "PortAllocatorService")
    init_app_config_event_store = _load_legacy_symbol("platform.config_center.runtime", "init_app_config_event_store")
    config_watcher_cls = _load_legacy_symbol("platform.config_center.runtime", "ConfigWatcherService")
    agent_aggregator_cls = _load_legacy_symbol("platform.registry.runtime", "AgentAggregatorService")
    skill_catalog_cls = _load_legacy_symbol("platform.registry.runtime", "SkillCatalogService")
    rag_overview_cls = _load_legacy_symbol("platform.registry.runtime", "RAGOverviewService")
    init_rag_config_store = _load_legacy_symbol("platform.config_center.runtime", "init_rag_config_store")
    mcp_registry_cls = _load_legacy_symbol("platform.registry.runtime", "MCPRegistryService")
    studio_service_cls = _load_legacy_symbol("platform.registry.runtime", "StudioService")
    tenant_invitation_cls = _load_legacy_symbol("platform.tenants.runtime", "TenantInvitationService")
    get_default_llm_management_service = _load_legacy_symbol(
        "platform.config_center.runtime",
    ...
    ensure_platform_db_ready = _load_legacy_symbol("platform.operations.db", "ensure_platform_db_ready")
    close_platform_db = _load_legacy_symbol("platform.operations.db", "close_platform_db")

    init_app_services = _load_legacy_symbol("platform.api.routers", "init_app_services")
    init_agent_services = _load_legacy_symbol("platform.api.routers", "init_agent_services")
    init_skill_services = _load_legacy_symbol("platform.api.routers", "init_skill_services")
    init_rag_services = _load_legacy_symbol("platform.api.routers", "init_rag_services")
    init_mcp_services = _load_legacy_symbol("platform.api.routers", "init_mcp_services")
    init_studio_services = _load_legacy_symbol("platform.api.routers", "init_studio_services")
    ...
    logger.info("Platform 起動完了: %d 件の App を検出", count)
        ...
        try:
            if args.reload:
                uvicorn.run(
                    ...
                    port=args.port,
                    reload=True,
                    reload_dirs=["platform", "apps/platform", "agentflow"],
                )
            else:
