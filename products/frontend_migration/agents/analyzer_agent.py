...
class AnalyzerAgent:
    ...
    def _register_analysis_tools(self) -> None:
        ...
        def analyze_css_file(file_path: str, content: str) -> List[Dict[str, Any]]:
            ...
            for line_num, line in enumerate(lines, 1):
                ...
                for property_name, px_value in px_matches:
                    ...
                    if property_name in ['width', 'height', 'max-width', 'max-height', 'font-size']:
                        issues.append({
                            ...
                            "suggested_fix": self._suggest_responsive_fix(property_name, px_value),
                            "auto_fixable": True
                        })
        ...
        def analyze_html_structure(file_path: str, content: str) -> List[Dict[str, Any]]:
            """
    ...
    async def analyze_file(self, file_path: str) -> List[CodeIssue]:
        ...
        if file_extension in ['.css', '.scss', '.sass']:
            result = await self.tool_manager.execute("analyze_css_file", {
                "file_path": file_path,
                "content": content
            })
            if result.success:
            ...
            result = await self.tool_manager.execute("analyze_js_file", {
                "file_path": file_path,
                "content": content
            })
            if result.success:
            ...
            result = await self.tool_manager.execute("analyze_html_structure", {
                "file_path": file_path,
                "content": content
            })
            if result.success:
    ...
    def _calculate_file_statistics(self, issues: List[CodeIssue]) -> Dict[str, Any]:
        ...
        for issue in issues:
            ...
            stats["issue_type_breakdown"][issue_type] = stats["issue_type_breakdown"].get(issue_type, 0) + 1

        return stats

    def _analyze_framework_issues(self, manifest: Dict[str, Any], issues: List[CodeIssue]) -> Dict[str, Any]:
        """フレームワーク関連の問題を分析"""
        framework_info = manifest.get("framework_info", {})

        analysis = {
            "jquery_migration_needed": any(issue.issue_type == IssueType.JQUERY_DEPENDENCY for issue in issues),
            "css_framework_conflicts": [],
            "js_modernization_needed": any(issue.issue_type == IssueType.PERFORMANCE_ISSUE for issue in issues),
            "accessibility_compliance": self._assess_accessibility_compliance(issues)
        }

        return analysis

    def _assess_accessibility_compliance(self, issues: List[CodeIssue]) -> Dict[str, Any]:
        """アクセシビリティ準拠状況を評価"""
        accessibility_issues = [issue for issue in issues if issue.issue_type == IssueType.ACCESSIBILITY_VIOLATION]

        return {
            "total_violations": len(accessibility_issues),
            "wcag_compliance_level": "AA" if len(accessibility_issues) == 0 else "Non-compliant",
            "critical_issues": [issue.description for issue in accessibility_issues if issue.severity == "high"]
        }
    ...
    def _assess_migration_complexity(self, issues: List[CodeIssue]) -> str:
        ...
        total_issues = len(issues)
        auto_fixable_ratio = sum(1 for issue in issues if issue.auto_fixable) / max(total_issues, 1)
    ...
    async def save_analysis_result(self, result: AnalysisResult, output_path: str) -> None:
        """
        解析結果をJSONファイルに保存
