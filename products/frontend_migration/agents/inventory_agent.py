...
class InventoryAgent:
    ...
    async def scan_repository(self, repo_path: str) -> RepositoryManifest:
        ...
        # ディレクトリスキャンを実行
        scan_result = await self.tool_manager.execute("scan_directory", {"directory_path": repo_path})
        ...
        # 依存関係抽出を実行
        deps_result = await self.tool_manager.execute("extract_dependencies", {"project_path": repo_path})
        ...
        logger.info(f"リポジトリスキャン完了: {manifest.total_files}個のファイル、{len(manifest.dependencies)}個の依存関係")
        return manifest
    
    def _detect_framework(self, files: List[FileInfo], dependencies: Dict[str, str]) -> Dict[str, Any]:
        """
        使用されているフレームワークを検出
    ...
    async def save_manifest(self, manifest: RepositoryManifest, output_path: str) -> None:
        """
        マニフェストをJSONファイルに保存
