...
class MigrationPlannerAgent:
    ...
    async def _analyze_tech_stack(
        ...
        # 問題の分析
        jquery_issues = len([i for i in issues if i.get("issue_type") == "jquery_dependency"])
        ie_issues = len([i for i in issues if i.get("issue_type") == "ie_compatibility"])
        px_issues = len([i for i in issues if i.get("issue_type") == "fixed_px_size"])
        
        # 推奨技術スタック
        recommended_tech = self._recommend_tech_stack(current_tech, {
    ...
    def _identify_page_risks(
        ...
        # jQuery依存のチェック
        jquery_issues = [i for i in page_issues if i.get("issue_type") == "jquery_dependency"]
        if jquery_issues:
            risks.append("jQuery依存の解消が必要")
        
        # アクセシビリティ問題のチェック
        a11y_issues = [i for i in page_issues if i.get("issue_type") == "accessibility_violation"]
        if a11y_issues:
    ...
    async def _assess_risks(
        ...
        high_priority_pages = len([p for p in page_plans if p.priority == MigrationPriority.CRITICAL])
    ...
    async def save_blueprint(self, blueprint: MigrationBlueprint, output_path: str) -> None:
        """
