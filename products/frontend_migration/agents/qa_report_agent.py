...
class QAReportAgent:
    ...
    def _register_qa_tools(self) -> None:
        ...
        def run_lighthouse_audit(page_url: str, categories: List[str] = None) -> Dict[str, Any]:
            """
            ...
            lighthouse_results = {
                "performance": {
                    "score": 0.85,
                    "metrics": {
                        "first-contentful-paint": {"score": 0.9, "value": 1.2, "unit": "s"},
                        "largest-contentful-paint": {"score": 0.8, "value": 2.1, "unit": "s"},
                        "cumulative-layout-shift": {"score": 0.95, "value": 0.05, "unit": ""},
                        "total-blocking-time": {"score": 0.7, "value": 150, "unit": "ms"}
                    },
                    "opportunities": [
                        "画像の最適化",
                        "未使用CSSの削除",
        ...
        def run_accessibility_audit(page_url: str, tools: List[str] = None) -> Dict[str, Any]:
            """
        ...
        def analyze_performance_metrics(lighthouse_data: Dict[str, Any]) -> List[Dict[str, Any]]:
            """
            パフォーマンスメトリクスを分析
    ...
    async def generate_qa_report(
        ...
        # Lighthouse 監査を実行
        lighthouse_result = await self.tool_manager.execute("run_lighthouse_audit", {
            "page_url": page_url,
            "categories": ["performance", "accessibility", "best-practices", "seo"]
        })
        
        # アクセシビリティ監査を実行
        a11y_result = await self.tool_manager.execute("run_accessibility_audit", {
            "page_url": page_url,
            "tools": ["axe-core", "wave"]
        })
        ...
        if lighthouse_result.success:
            ...
            # パフォーマンスメトリクス分析
            perf_analysis_result = await self.tool_manager.execute("analyze_performance_metrics", {
            ...
            accessibility_metrics.append(QualityMetric(
            ...
            seo_metrics.append(QualityMetric(
                name="SEO Score",
            ...
            best_practices.append(QualityMetric(
        ...
        category_scores = {
            ...
            "accessibility": accessibility_metrics[0].score if accessibility_metrics else 0,
            "seo": seo_metrics[0].score if seo_metrics else 0,
            "best_practices": best_practices[0].score if best_practices else 0
        }
        
        overall_score_result = await self.tool_manager.execute("calculate_overall_score", {
            "category_scores": category_scores
        })
        
        overall_score = overall_score_result.result if overall_score_result.success else 0
        
        # 改善計画を生成
        improvement_plan_result = await self.tool_manager.execute("generate_improvement_plan", {
            "qa_results": {
        ...
        recommendations = improvement_plan_result.result if improvement_plan_result.success else []
        
        # 移行前後の比較データ
        before_after_comparison = None
        if before_data:
            before_after_comparison = {
                "performance_improvement": category_scores["performance"] - before_data.get("performance", 0),
                "accessibility_improvement": category_scores["accessibility"] - before_data.get("accessibility", 0),
