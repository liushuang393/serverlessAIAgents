...
class TestAgent:
    ...
    def _register_test_tools(self) -> None:
        ...
        def generate_unit_tests(component_code: str, component_name: str) -> List[Dict[str, str]]:
            """
...
"""
        ...
        def generate_integration_tests(component_name: str, dependencies: List[str]) -> List[Dict[str, str]]:
            """
            インテグレーションテストを生成
...
"""
        ...
        def generate_test_setup(component_name: str, dependencies: List[str]) -> Dict[str, str]:
            """
            テストセットアップコードを生成
...
"""
    ...
    async def generate_test_suite(
        ...
        test_types = test_types or ["unit", "accessibility", "integration", "performance"]
        ...
        for test_type in test_types:
            if test_type == "unit":
                unit_result = await self.tool_manager.execute("generate_unit_tests", {
                ...
                a11y_result = await self.tool_manager.execute("generate_accessibility_tests", {
                    "component_name": component_name
                })
                ...
                integration_result = await self.tool_manager.execute("generate_integration_tests", {
                    "component_name": component_name,
                ...
                perf_result = await self.tool_manager.execute("generate_performance_tests", {
                    "component_name": component_name
                })
        ...
        # セットアップコードを生成
        setup_result = await self.tool_manager.execute("generate_test_setup", {
            "component_name": component_name,
        ...
        # テストファイルパスを決定
        test_file_path = str(Path(component_file_path).parent / f"{component_name}.test.tsx")
