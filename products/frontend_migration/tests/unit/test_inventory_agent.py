...
$(document).ready(function() {
    ...
    async def test_scan_directory_tool(self, agent, sample_repo):
        """ディレクトリスキャンツールのテスト"""
        result = await agent.tool_manager.execute("scan_directory", {
            "directory_path": sample_repo
        })
    ...
    async def test_extract_dependencies_tool(self, agent, sample_repo):
        """依存関係抽出ツールのテスト"""
        result = await agent.tool_manager.execute("extract_dependencies", {
            "project_path": sample_repo
        })
...
class TestInventoryAgentPerformance:
    ...
    async def test_large_repository_scan(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(f"大規模スキャンパフォーマンス: {manifest.total_files}ファイルを{scan_duration:.2f}秒でスキャン")
