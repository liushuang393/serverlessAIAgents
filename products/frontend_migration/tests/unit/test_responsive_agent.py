...
"""
    ...
    async def test_convert_px_to_responsive_tool(self, agent):
        """px値レスポンシブ変換ツールのテスト"""
        # レイアウト幅の変換テスト
        result = await agent.tool_manager.execute("convert_px_to_responsive", {
            "property_name": "width",
            "px_value": "800",
            "context": "layout"
        })
    ...
    async def test_small_value_conversion(self, agent):
        ...
        # 10px = 0.625rem
        assert "0.62rem" in data["responsive_value"] or "0.63rem" in data["responsive_value"]
        assert data["unit_type"] == ResponsiveUnit.REM.value
    ...
    async def test_generate_media_queries_tool(self, agent):
        ...
        result = await agent.tool_manager.execute("generate_media_queries", {
            "css_rules": css_rules,
    ...
    async def test_convert_to_grid_layout_tool(self, agent):
        """Grid レイアウト変換ツールのテスト"""
        result = await agent.tool_manager.execute("convert_to_grid_layout", {
    ...
    async def test_convert_to_flexbox_layout_tool(self, agent):
        ...
        result = await agent.tool_manager.execute("convert_to_flexbox_layout", {
            "html_structure": "<div class='container'><div class='item'>Content</div></div>",
    ...
    async def test_convert_css_file(self, agent, sample_css_file):
        ...
        assert "rem" in conversion.responsive_css or "clamp" in conversion.responsive_css
    ...
    async def test_memory_integration(self, agent, sample_css_file):
        ...
        await agent.memory.store(
            ...
            metadata={"test": "responsive_memory_integration"}
        )

        # テストデータが検索できることを確認
        test_search = await agent.memory.search(
            query="Test responsive conversion memory",
            limit=1
        )
...
"""
        ...
        with tempfile.NamedTemporaryFile(mode='w', suffix='.css', delete=False, encoding='utf-8') as f:
            ...
            try:
                ...
                assert conversion_duration < 10.0, f"変換時間が長すぎます: {conversion_duration:.2f}秒"
                assert len(conversion.applied_rules) > 0
                
                print(f"大規模変換パフォーマンス: {len(issue_list)}問題を{conversion_duration:.2f}秒で変換")
