...
class MigrationOrchestrator:
    ...
    def _setup_routing_rules(self) -> None:
        ...
        # インベントリ → アナライザー
        self.router.register_route(RouteDefinition(
    ...
    async def migrate_project(self, project_path: str) -> MigrationResult:
        ...
        try:
            ...
            analysis_result = await self.analyzer_agent.analyze_project(str(manifest_path))
            analysis_path = output_dir / "issue_list.json"
            await self.analyzer_agent.save_analysis_result(analysis_result, str(analysis_path))
            ...
            migration_result = MigrationResult(
                ...
                issues_found=len(analysis_result.issues),
                issues_fixed=issues_fixed,
                conversion_success_rate=issues_fixed / max(len(analysis_result.issues), 1),
            ...
            if self.config["generate_reports"]:
                report_paths = await self._generate_final_reports(
                    migration_result, manifest, analysis_result, conversion_results, output_dir
                )
    ...
    async def _generate_final_reports(
        ...
        summary_report = {
            ...
            "migration_summary": {
                ...
                "duration_seconds": (migration_result.end_time - migration_result.start_time).total_seconds(),
