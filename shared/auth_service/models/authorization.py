...
class RolePermission(Base):
    ...
    permission_id: Mapped[str] = mapped_column(
        String(64), ForeignKey("permissions.id", ondelete="CASCADE"), nullable=False,
    )
    created_at: Mapped[datetime] = mapped_column(
        DateTime, server_default=text("CURRENT_TIMESTAMP"), nullable=False,
    )
...
class ResourceDefinition(Base):
    ...
    __table_args__ = (
        UniqueConstraint("resource_type", "resource_id", name="uq_resource_def_type_id"),
    )
...
class ResourcePermission(Base):
    ...
    __table_args__ = (
        Index("ix_resource_permission_role_type_id", "role_id", "resource_type", "resource_id"),
    )
    ...
    user_id: Mapped[str | None] = mapped_column(
        String(64), ForeignKey("user_accounts.id", ondelete="CASCADE"), nullable=True,
    )
    resource_type: Mapped[str] = mapped_column(String(100), nullable=False, index=True)
    resource_id: Mapped[str] = mapped_column(String(255), nullable=False, index=True)
