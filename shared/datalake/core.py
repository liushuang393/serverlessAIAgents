...
使用例:
    >>> from shared.datalake import DataLake, get_datalake
    >>>
    >>> # シングルトン取得
    >>> dl = get_datalake()
    >>>
...
from shared.datalake.auth import AuthProvider
from shared.datalake.connector import DataConnector
from shared.datalake.format_handlers import get_format_handler
...
class ReadResult(BaseModel):
    ...
    content: bytes | str | dict[str, Any] | list[Any] | Any = Field(description="コンテンツ")
...
def _register_default_connectors(datalake: DataLake) -> None:
    ...
    try:
        from shared.datalake.connectors.s3 import S3Connector
    ...
    try:
        from shared.datalake.connectors.rest import RestAPIConnector
