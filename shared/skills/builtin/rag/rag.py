...
class RAGSkill:
    ...
    async def add_document(
        ...
        ids = await self.rag_service.add_documents(
            documents=[content], metadatas=[{"topic": topic, **(metadata or {})}]
        )
        return ids[0]
    ...
    async def query(
        ...
        # 1. 検索
        docs = await self.rag_service.retrieve(query=question, top_k=self._rag_config.top_k, filter=filter)
        ...
        answer = await self.rag_service.query(
            query=question, top_k=self._rag_config.top_k, filter=filter, system_prompt=self._rag_config.system_prompt
        )

        return RAGResult(answer=answer, sources=docs, context_used=context)
