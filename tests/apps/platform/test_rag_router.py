...
class TestGetRagOverview:
    """GET /api/rag/overview テスト."""

    def test_returns_overview(self, phase3_test_client: TestClient) -> None:
        """RAG 概要を返す."""
        resp = phase3_test_client.get("/api/rag/overview")
...
class TestListStrategies:
    """GET /api/rag/strategies テスト."""

    def test_returns_strategies(self, phase3_test_client: TestClient) -> None:
        """チャンキング戦略一覧を返す."""
        resp = phase3_test_client.get("/api/rag/strategies")
        assert resp.status_code == 200
        data = resp.json()
...
class TestListRerankers:
    """GET /api/rag/rerankers テスト."""

    def test_returns_rerankers(self, phase3_test_client: TestClient) -> None:
        """リランカー一覧を返す."""
        resp = phase3_test_client.get("/api/rag/rerankers")
        assert resp.status_code == 200
        data = resp.json()
        assert "rerankers" in data
        assert "total" in data
...
class TestRetrievalMethods:
    """GET /api/rag/retrieval-methods テスト."""

    def test_returns_methods(self, phase3_test_client: TestClient) -> None:
        """検索方式一覧を返す."""
        resp = phase3_test_client.get("/api/rag/retrieval-methods")
        assert resp.status_code == 200
        data = resp.json()
...
class TestPatterns:
    """GET /api/rag/patterns テスト."""

    def test_returns_patterns(self, phase3_test_client: TestClient) -> None:
        """RAG パターン一覧を返す."""
        resp = phase3_test_client.get("/api/rag/patterns")
        assert resp.status_code == 200
        data = resp.json()
...
class TestRagAppConfigs:
    """GET/PATCH /api/rag/apps/{app}/config テスト."""

    def test_list_configs(self, phase3_test_client: TestClient) -> None:
        """全 App の RAG 設定一覧を返す."""
        resp = phase3_test_client.get("/api/rag/apps/configs")
        assert resp.status_code == 200
        data = resp.json()
        assert "apps" in data
        assert "total" in data
    ...
    def test_get_single_config(self, phase3_test_client: TestClient) -> None:
        """単一 App の RAG 設定を返す."""
        resp = phase3_test_client.get("/api/rag/apps/rag_app/config")
        assert resp.status_code == 200
        data = resp.json()
    ...
    def test_patch_missing_app_returns_404(self, phase3_test_client: TestClient) -> None:
        """存在しない App は 404."""
        resp = phase3_test_client.patch(
            "/api/rag/apps/does_not_exist/config",
            json={"enabled": True},
        )
...
class TestGetRagStats:
    """GET /api/rag/stats テスト."""

    def test_returns_stats(self, phase3_test_client: TestClient) -> None:
        """RAG 統計を返す."""
        resp = phase3_test_client.get("/api/rag/stats")
        assert resp.status_code == 200
        data = resp.json()
