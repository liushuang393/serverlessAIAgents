...
from ai_blocks.core.registry import get_registry, DeploymentStrategy
...
class ChaosMemory(VectorMemory):
    ...
    async def search(self, query: str, limit: int = 10, threshold: float = None) -> List:
        """カオス注入付き検索操作"""
        # ネットワーク分断シミュレート
...
class TestChaosEngineering:
    ...
    async def test_memory_chaos_resilience(self):
        ...
        # 結果の検証
        total_operations = successful_stores + failed_stores + successful_searches + failed_searches
            ...
            for tool_name, params in tools_to_test:
                try:
                    ...
                    result = await asyncio.wait_for(
                        chaos_tool_manager.execute(tool_name, params),
                        timeout=5.0
                    )
    ...
    async def test_concurrent_chaos_operations(self):
        ...
        async def tool_worker(worker_id: int):
            ...
            for i in range(10):
                try:
                    result = await chaos_tool_manager.execute("echo", {"text": f"ワーカー{worker_id}_{i}"})
                    if result.success:
                        results["success"] += 1
                    else:
                        results["failure"] += 1
        ...
        # 結果の集計
        total_memory_success = sum(r["success"] for r in memory_results if isinstance(r, dict))
        total_memory_failure = sum(r["failure"] for r in memory_results if isinstance(r, dict))
        total_tool_success = sum(r["success"] for r in tool_results if isinstance(r, dict))
        total_tool_failure = sum(r["failure"] for r in tool_results if isinstance(r, dict))
    ...
    async def test_cascading_failure_resilience(self):
        ...
        async def complex_workflow(workflow_id: int):
            ...
            try:
                ...
                # 2. ツールを実行
                tool_result = await chaos_tool_manager.execute("echo", {"text": f"ワークフロー{workflow_id}"})
                ...
                message = Message(
                    ...
                    content=f"ワークフロー{workflow_id}完了"
                )
            ...
            for i in range(30):
                ...
                try:
                    ...
                    self.observability.record_metric(
                        "chaos_test_operations", 1.0, "counter",
                    ...
                    # 成功メトリクスを記録
                    self.observability.record_metric(
                        "chaos_test_success", 1.0, "counter",
                        {"operation": "store"}
                    )
                    ...
                    # 失敗メトリクスを記録
                    self.observability.record_metric(
                        "chaos_test_failures", 1.0, "counter",
                        {"operation": "store"}
                    )
