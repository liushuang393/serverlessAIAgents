...
async function mockLLMSuccessRoutes(page: Page): Promise<void> {
  await page.route('**/openapi.json', async (route) => {
    await route.fulfill({
      ...
      body: JSON.stringify({ paths: { '/api/studios/framework/llm/overview': {} } }),
    });
  });
  await page.route('**/api/studios/framework/llm/**', async (route) => {
    ...
    if (url.endsWith('/engines/status')) {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ engine_status: [] }) });
      return;
    }
    if (url.endsWith('/reload')) {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ reloaded: true, gateway: overviewPayload.gateway }),
      });
      return;
    }
...
async function mockLLMMissingRoute(page: Page): Promise<void> {
  ...
  await page.route('**/api/studios/framework/llm/diagnostics', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({
...
test.describe('LLM Management E2E', () => {
  test('setup-and-switch success flow (mock backend)', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="llm-switch-panel"]')).toBeVisible();

    await page.fill('[data-testid="llm-switch-model"]', 'gpt-4o-mini');
    await page.click('[data-testid="llm-setup-switch-button"]');

    await expect(page.locator('[data-testid="llm-setup-switch-success"]')).toBeVisible();
    await expect(page.locator('[data-testid="llm-setup-switch-success"]')).toContainText('Switch completed successfully');
  });

  test('shows route missing diagnostics when overview returns 404', async ({ page }) => {
    await mockLLMMissingRoute(page);
    await page.goto('http://localhost:3000/llm-management');

    const diagnostic = page.locator('[data-testid="llm-route-missing-diagnostic"]');
