...
test.describe("RAG Overview - パターン選択 E2E", () => {
  ...
  test('パターン "faq_precision" を選択するとパラメータが自動セットされる', async ({
    page,
  }) => {
    // RAG 設定画面を開く
    ...
    // 自動セットされたパラメータを確認
    await expect(
      page.locator('[data-testid="chunk-strategy-select"]'),
    ).toHaveValue("sentence");
    await expect(page.locator('[data-testid="chunk-size-input"]')).toHaveValue(
      "500",
    );
    await expect(
      page.locator('[data-testid="chunk-overlap-input"]'),
    ).toHaveValue("80");
    await expect(
      page.locator('[data-testid="retrieval-method-select"]'),
    ).toHaveValue("hybrid");
    await expect(page.locator('[data-testid="top-k-input"]')).toHaveValue("8");

    // modified バッジが表示されないこと（パターン直後はクリーン状態）
    await expect(
      page.locator('[data-testid="rag-pattern-modified-badge"]'),
    ).not.toBeVisible();
  });

  test("パターン選択後にパラメータを手動上書きすると modified バッジが表示される", async ({
    page,
  }) => {
    await page.click('[data-testid="nav-rag"]');
    await page.waitForSelector('[data-testid^="rag-app-card-"]');
