...
from ai_blocks.core.registry import (
    get_registry, register_component, DeploymentStrategy, HealthCheckResult, ComponentStatus
)
from ai_blocks.core.factory import get_factory, get_architecture_factory
    ...
    async def search(self, query: str, limit: int = 10, threshold: float = None) -> list[MemoryItem]:
        ...
        for item in self._items.values():
            if query.lower() in item.content.lower():
                item.similarity_score = 0.95 if self._performance_boost else 0.8  # 改良版では高い類似度
                results.append(item)
...
class TestHotswapIntegration:
    ...
    async def test_deployment_strategies_comparison(self):
        ...
        assert deployment_times[DeploymentStrategy.IMMEDIATE] <= min(deployment_times.values())
    ...
    async def test_factory_dependency_resolution(self):
        ...
        memory = await self.factory.create(
            "memory", "test_vector", "1.0.0",
            max_items=500, similarity_threshold=0.8
        )
