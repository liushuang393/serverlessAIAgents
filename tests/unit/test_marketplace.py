...
LocalRegistry のロジックテストと MarketplaceClient の統合テスト。
外部 API への HTTP 通信はモックで置換する。
"""
...
from platform.marketplace import AgentRegistryEntry, LocalRegistry, MarketplaceClient
from platform.marketplace.client import MarketplaceAgent
...
def _patch_marketplace_http():
    """MarketplaceClient テスト用の HTTP モック."""
    with (
        patch.object(
            MarketplaceClient,
